...
func (s *Service) DeleteMediaTypeRuleInSpace(spaceID, id string) error {
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("space_id = ? AND id = ?", spaceID, ruleID).First(&rule).Error; err != nil {
			return err
		}
		if rule.Builtin {
			return fmt.Errorf("内置规则不可删除")
...
func (s *Service) applyLegacyMediaExtensions(rules map[string]MediaTypeRuleView, libraryID int64) error {
	...
	if !s.mediaExtensionTableExists() {
		return nil
	}
...
func (s *Service) mediaTypeRuleTableExists() bool {
	return s.db != nil && s.db.Migrator().HasTable(&models.MediaTypeRule{})
}

func (s *Service) mediaExtensionTableExists() bool {
	return s.db != nil && s.db.Migrator().HasTable(&models.MediaExtension{})
}

func (s *Service) loadPersistedMediaTypeRules(spaceID string, libraryID int64) ([]models.MediaTypeRule, error) {
	var rows []models.MediaTypeRule
	query := s.db.Where("space_id = ? AND library_id IS NULL", normalizeSpaceID(spaceID))
	if libraryID > 0 {
		query = s.db.Where("space_id = ? AND (library_id IS NULL OR library_id = ?)", normalizeSpaceID(spaceID), libraryID)
	}
...
func (s *Service) upsertMediaTypeRule(spaceID string, libraryID *int64, mediaType, ext string, enabled, builtin bool, label, description, action string) (*models.MediaTypeRule, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		...
		existing, err := findMediaTypeRule(tx, spaceID, libraryID, mediaType, ext)
		...
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			rule.ID = existing.ID
			if err := tx.Model(existing).Updates(mediaTypeRuleUpdates(rule)).Error; err != nil {
				return err
			}
			saved = *existing
...
func (s *Service) updatePersistedMediaTypeRule(spaceID, id string, input MediaTypeRuleUpdate) (MediaTypeRuleView, error) {
	...
	err = s.db.Transaction(func(tx *gorm.DB) error {
		...
		if len(updates) > 0 {
			...
			if err := tx.First(&rule, rule.ID).Error; err != nil {
				return err
			}
		}
		saved = rule
...
func findMediaTypeRule(tx *gorm.DB, spaceID string, libraryID *int64, mediaType, ext string) (*models.MediaTypeRule, error) {
	var rule models.MediaTypeRule
	query := tx.Where("space_id = ? AND type = ? AND extension = ?", spaceID, mediaType, ext)
	if libraryID == nil || *libraryID <= 0 {
