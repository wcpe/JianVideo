...
func (s *Service) claimRecycleMedia(spaceID string, snapshot models.MediaFile) (recycleClaim, bool, error) {
	...
	err := s.db.Transaction(func(tx *gorm.DB) error {
		current, found, err := findRecycleClaimCandidate(tx, spaceID, snapshot)
		if err != nil || !found {
			return err
		}
		...
		result := tx.Model(&models.MediaFile{}).
			Where("space_id = ? AND id = ? AND deleted_at = ? AND file_state = ?", spaceID, current.ID, current.DeletedAt, current.FileState).
			UpdateColumn("file_state", claim.state)
		if result.Error != nil {
			return result.Error
		}
...
func findRecycleClaimCandidate(tx *gorm.DB, spaceID string, snapshot models.MediaFile) (*models.MediaFile, bool, error) {
	var current models.MediaFile
	err := tx.Where("space_id = ? AND id = ? AND deleted_at = ?", spaceID, snapshot.ID, snapshot.DeletedAt).First(&current).Error
...
func (s *Service) releaseRecycleClaim(claim recycleClaim) error {
	result := s.db.Model(&models.MediaFile{}).
		Where("space_id = ? AND id = ? AND deleted_at = ? AND file_state = ?", claim.media.SpaceID, claim.media.ID, claim.media.DeletedAt, claim.state).
		UpdateColumn("file_state", claim.originalFileState)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected != 1 {
		return fmt.Errorf("释放回收站清理 claim 影响行数异常: mediaID=%d, rows=%d", claim.media.ID, result.RowsAffected)
	}
	return nil
}
