...
const QUALITY_1080: PlaybackQuality = {
  bandwidth: 5_000_000,
  height: 1080,
  id: "1080-high",
  label: "1080p",
};
const QUALITY_720: PlaybackQuality = {
  bandwidth: 2_500_000,
  height: 720,
  id: "720-high",
  label: "720p",
};
const QUALITY_480_LOW: PlaybackQuality = {
  bandwidth: 800_000,
  height: 480,
  id: "480-low",
  label: "480p · 800 kbps",
};
const QUALITY_480_HIGH: PlaybackQuality = {
  bandwidth: 1_200_000,
  height: 480,
  id: "480-high",
  label: "480p · 1200 kbps",
};
const QUALITY_360: PlaybackQuality = {
  bandwidth: 500_000,
  height: 360,
  id: "360",
  label: "360p",
...
class FakeQualityFacet implements QualityFacet {
  ...
  selectQuality(
    ...
    if (this.selectFailure !== undefined)
      return Promise.reject(this.selectFailure);
  ...
  replaceState(
    state: Partial<QualityFacetState>,
    command: PlaybackCommandContext,
  ): void {
...
function availableCapabilities() {
  return {
    ...EMPTY_CAPABILITIES,
    loadControl: "available" as const,
...
async function createLoadedCore(
  ...
  if (options.qualities !== undefined) {
    quality.replaceState(
      ...
      { requestId: 0, sourceEpoch: 0, sourceId: SOURCE_A.id },
    );
  }
...
describe("PlaybackCore 清晰度与省流量", () => {
  ...
  it("手动选择使用高度和带宽语义，切回自动不会把实际档位误标为手动", async () => {
    ...
    await expect(
      core.selectQuality(manualQuality(QUALITY_480_LOW)),
    ...
    expect(quality.calls.find((call) => call.type === "select")).toMatchObject({
      selection: {
        mode: "manual",
        quality: { bandwidth: 800_000, height: 480 },
      },
      type: "select",
    });
    expect(core.getQualityState()).toMatchObject({
      manualQuality: QUALITY_480_LOW,
      qualityMode: "manual",
    });
  ...
  it("自动省流量把 ABR 上限锁到 480p 最高可用变体", async () => {
    ...
    await expect(core.setDataSaver(true)).resolves.toMatchObject({
      status: "completed",
    });

    expect(quality.calls).toContainEqual(
      expect.objectContaining({ maxHeight: 480, type: "cap" }),
    );
    expect(core.getQualityState()).toMatchObject({
      dataSaver: true,
      dataSaverBlocked: false,
  ...
  it("手动低档开启省流量保持原档，手动高档开启时降到 480p 最高带宽变体", async () => {
    ...
    expect(low.core.getQualityState()).toMatchObject({
      manualQuality: QUALITY_360,
      qualityMode: "manual",
    });
    ...
    expect(high.core.getQualityState()).toMatchObject({
      manualQuality: QUALITY_480_HIGH,
      qualityMode: "manual",
    });
  });
  ...
  it("省流量中手动选择高档先清除上限并关闭省流量", async () => {
    ...
    expect(quality.calls.slice(0, 2).map((call) => call.type)).toEqual([
      "cap",
      "select",
    ]);
  ...
  it("省流量切换新源时空清单不阻断，后续兼容档位解除已确认阻断", async () => {
    ...
    quality.replaceState(
      {
        actualQualityId: QUALITY_480_LOW.id,
        qualities: [QUALITY_1080, QUALITY_480_LOW],
      },
      currentCommand(core),
    );
    await flushTasks();
    ...
    expect(
      quality.calls.filter((call) => call.type === "cap").at(-1),
    ).toMatchObject({ maxHeight: 480 });
  });
  ...
  it("切换失败保持原核心状态且不破坏播放", async () => {
    ...
    quality.selectFailure = new PlaybackBackendError(
      "media",
      "目标清晰度切换失败",
    );

    await expect(
      core.selectQuality(manualQuality(QUALITY_480_HIGH)),
    ).resolves.toMatchObject({
...
describe("PlaybackCore 倍速", () => {
  it("只接受固定七档并仅调用 QualityFacet.setPlaybackRate", async () => {
    ...
    for (const rate of [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2] as const) {
      await expect(core.setPlaybackRate(rate)).resolves.toMatchObject({
        status: "completed",
      });
    }
    expect(
      quality.calls
        .filter((call) => call.type === "rate")
        .map((call) => call.rate),
    ).toEqual([0.5, 0.75, 1, 1.25, 1.5, 1.75, 2]);
    await expect(core.setPlaybackRate(2.5)).resolves.toMatchObject({
      status: "unsupported",
    });
  });
  ...
  it("后端拒绝倍速时回退 1x，新媒体也重置 1x", async () => {
    ...
    quality.rateFailure = new PlaybackBackendError(
      "unsupported",
      "不支持该倍速",
    );

    await expect(core.setPlaybackRate(2)).resolves.toMatchObject({
      status: "unsupported",
    });
  ...
  it("同 sourceId 重新加载后恢复原倍速而不是保留后端重置的 1x", async () => {
    ...
    expect(
      quality.calls.filter((call) => call.type === "rate").at(-1),
    ).toMatchObject({ rate: 1.5 });
...
describe("PlaybackCore A-B 循环", () => {
  it("A 点单独不启用，合法 B 点启用且最小区间为 0.5 秒", async () => {
    ...
    await expect(core.setAbLoopB()).resolves.toMatchObject({
      status: "unsupported",
    });
    expect(core.getAbLoopState()).toEqual({ a: 10, b: null, enabled: false });
  ...
  it("仅在实际 currentTime 大于等于 B 时以 ab_loop 原因回跳 A", async () => {
    ...
    const completions: Array<
      Extract<PlaybackEvent, { readonly type: "seekCompleted" }>
    > = [];
    ...
    await flushTasks();
    expect(backend.seekRequests.at(-1)).toMatchObject({
      reason: "ab_loop",
      targetTime: 5,
    });
  ...
  it("用户 seek 到 B 后立即回 A，seek 到 A 前不清除区间", async () => {
    ...
    expect(
      backend.seekRequests.slice(-2).map((request) => request.reason),
    ).toEqual(["user", "ab_loop"]);
