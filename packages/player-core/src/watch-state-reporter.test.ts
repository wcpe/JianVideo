...
function sentCall(
  send: SendMock,
  index: number,
): Parameters<WatchStateTransport["send"]> {
...
describe("WatchStateReporter", () => {
  ...
  it("后台冲突、关闭补报和旧 session 均不重试", async () => {
    ...
    const closingSend = vi
      .fn<WatchStateTransport["send"]>()
      .mockResolvedValue(conflict(3, 60));
  ...
  it("不同 reporter 创建不同 session，旧 reporter 的迟到结果不能污染新媒体状态", async () => {
    ...
    const newSend = vi.fn<WatchStateTransport["send"]>((event) =>
      Promise.resolve(applied(event, 6)),
    );
    ...
    expect(newReporter.getState()).toEqual({
      completed: false,
      positionSeconds: 70,
      revision: 6,
    });
