...
describe("PlaybackCore 阶梯 Seek", () => {
  ...
  it("显式初始档位可读，切档不位移且非法档位受控拒绝", async () => {
    ...
    await expect(core.setSeekTier(invalid)).resolves.toMatchObject({
      status: "unsupported",
    });
    expect(core.getSeekTier()).toEqual({ kind: "seconds", value: 30 });
    expect(events).toEqual([
      { requestId: result.requestId, tier: { kind: "seconds", value: 30 } },
    ]);
  });

  it.each([0.5, 1, 5, 30, 60] as const)(
    "秒级 %s 档前后完全对称",
    async (value) => {
      ...
      await expect(core.seekByTier("next")).resolves.toMatchObject({
        confirmedTime: 80 + value,
        status: "completed",
      });
      await expect(core.seekByTier("previous")).resolves.toMatchObject({
        confirmedTime: 80,
        status: "completed",
      });
  ...
  it("已在方向边界时直接完成且不调用后端 seek", async () => {
    const { backend, core } = createTierCore(
      { kind: "seconds", value: 60 },
      200,
    );
    await core.load(SOURCE);
  ...
  it("快速 50 次秒级意图只允许最终结果回写", async () => {
    ...
    const requests = Array.from({ length: 50 }, (_, index) =>
      core.seekByTier(index === 49 ? "next" : "previous"),
    );
    ...
    expect(
      results.slice(0, -1).every(({ status }) => status === "superseded"),
    ).toBe(true);
