...
describe("PlaybackCore", () => {
  it("完成 load→ready→play→pause 并分配单调 requestId", async () => {
    ...
    await expect(core.play()).resolves.toMatchObject({
      requestId: 2,
      status: "completed",
    });
    expect(core.getSnapshot().state).toBe("playing");
    await expect(core.pause()).resolves.toMatchObject({
      requestId: 3,
      status: "completed",
    });
  ...
  it("持有 PreviewFacet 并通过核心绑定、命中当前源预览轨", async () => {
    ...
    const core = new PlaybackCore({
      backend,
      facets: { preview: createPreviewFacet() },
    });
    const load = await core.load(SOURCE_A);
    const command: PlaybackCommandContext = {
  ...
  it("将 seek 目标夹取到可 Seek 区间边界", async () => {
    ...
    await expect(core.seek(-10)).resolves.toMatchObject({
      clamped: true,
      confirmedTime: 2,
      targetTime: 2,
    });
    await expect(core.seek(99)).resolves.toMatchObject({
      clamped: true,
      confirmedTime: 8,
      targetTime: 8,
    });
  ...
  it("快速双 seek 以最后意图为准且旧结果不覆盖快照", async () => {
    ...
    backend.seekHandler = () => {
      seekCount += 1;
      return seekCount === 1
        ? firstBackendResult.promise
        : secondBackendResult.promise;
    };
    const core = new PlaybackCore({ backend });
    await core.load(SOURCE_A);
  ...
  it("stop 暂停并回到可定位起点，重复调用不重复触发后端", async () => {
    ...
    await expect(core.stop()).resolves.toMatchObject({
      requestId: 2,
      status: "completed",
    });
    expect(core.getSnapshot()).toMatchObject({
  ...
  it("切源会受控取代进行中的 stop，旧源不得继续 seek", async () => {
    ...
    expect(backend.calls.filter((call) => call.method === "seek")).toHaveLength(
      0,
    );
  ...
  it("seekBy 拒绝不属于当前源的后端实时快照", async () => {
    ...
    const seekCalls = backend.calls.filter(
      (call) => call.method === "seek",
    ).length;
  ...
  it("保留后端错误类别并把未分类异常归为 unknown", async () => {
    ...
    await core.load(SOURCE_A);
    backend.playHandler = () =>
      Promise.reject(new PlaybackBackendError("network", "网络中断"));

    await expect(core.play()).resolves.toMatchObject({ status: "failed" });
    expect(core.getSnapshot().error).toMatchObject({
      category: "network",
      message: "网络中断",
    });
  ...
  it("只接受当前源的 capabilitiesChanged 事件", async () => {
    const backend = new FakePlaybackBackend();
    const core = new PlaybackCore({ backend });
    const currentCapabilities: PlaybackCapabilities = {
      ...EMPTY_CAPABILITIES,
      preview: "available",
    };
    const staleCapabilities: PlaybackCapabilities = {
      ...EMPTY_CAPABILITIES,
      quality: "available",
    };
    await core.load(SOURCE_A);
  ...
  it.each([
    ...
    "pending play(r2) 期间的 $type(r2) 使用终态事件代次并拒绝旧事件",
    async (scenario) => {
      const backend = new FakePlaybackBackend();
      const pendingPlay = new Deferred<void>();
      backend.playHandler = () => pendingPlay.promise;
      ...
      const terminalSnapshots: Array<{
        readonly eventRequestId: number;
      ...
      core.subscribe((event) => {
        if (
          ...
          event.snapshot.state === scenario.expectedState
        ) {
          terminalSnapshots.push({
            eventRequestId: event.requestId,
            snapshotRequestId: event.snapshot.requestId,
          });
        }
      ...
      expect(terminalSnapshots).toEqual([
        { eventRequestId: 2, snapshotRequestId: 2 },
      ]);
  ...
  it("ended 后 seek 仍在片尾时保持 ended", async () => {
    ...
    await expect(core.seek(60)).resolves.toMatchObject({
      confirmedTime: 60,
      status: "completed",
    });
  ...
  it("快照与命令完成事件顶层 requestId 来自对应嵌套字段", async () => {
    ...
    for (const event of events) {
      expect(Number.isInteger(event.requestId) && event.requestId >= 0).toBe(
        true,
      );
      if (event.type === "snapshotChanged") {
  ...
  it("并发命令的完成事件各自携带对应 requestId 快照", async () => {
    ...
    const core = new PlaybackCore({ backend });
    const completions: Array<
      Extract<PlaybackEvent, { readonly type: "commandCompleted" }>
    > = [];
    core.subscribe((event) => {
      if (event.type === "commandCompleted") {
    ...
    expect(seekCompletion?.snapshot).toMatchObject({
      requestId: 2,
      state: "seeking",
    });
    expect(pauseCompletion?.snapshot).toMatchObject({
      requestId: 3,
      state: "paused",
    });
    pendingSeek.resolve(completedSeek(2, 8));
  ...
  it("idle 状态拒绝 play、pause、seek 且不触碰后端和快照", async () => {
    ...
    await expect(core.seek(1)).resolves.toMatchObject({
      status: "unsupported",
    });
  ...
  it("旧 load 晚成功不得覆盖新 load", async () => {
    const backend = new FakePlaybackBackend();
    const pendingLoad = new Deferred<void>();
    backend.loadHandler = (source) =>
      source.id === SOURCE_A.id ? pendingLoad.promise : Promise.resolve();
    const core = new PlaybackCore({ backend });
  ...
  it("先登记 pending 再惰性启动后端，重入 load 不会晚启动旧 load", async () => {
    ...
    core.subscribe((event) => {
      if (
        event.type === "snapshotChanged" &&
        event.snapshot.sourceId === SOURCE_A.id
      ) {
  ...
  it("发布重入后中止向后续 listener 传播过期快照", async () => {
    ...
    core.subscribe((event) => {
      if (event.type === "snapshotChanged") {
        laterListenerSources.push(
          `${String(event.snapshot.sourceId)}:${event.snapshot.state}`,
        );
      }
    });
  ...
  it.each(["superseded", "canceled", "unsupported"] as const)(
    ...
    async (status) => {
      ...
      const error =
        status === "unsupported"
          ? { category: "unsupported" as const, message: "不支持定位" }
          : undefined;
  ...
  it("unsupported rejection 让 load 与 seek 退出中间态并保留可见错误", async () => {
    ...
    const core = new PlaybackCore({ backend });
    backend.loadHandler = () =>
      Promise.reject(new PlaybackBackendError("unsupported", "源不受支持"));
    ...
    await core.pause();
    backend.seekHandler = () =>
      Promise.reject(new PlaybackBackendError("unsupported", "无法定位"));
    await expect(core.seek(4)).resolves.toMatchObject({
      error: { category: "unsupported" },
      status: "unsupported",
    });
    expect(core.getSnapshot()).toMatchObject({
  ...
  it("空 seekable 区间不调用后端并返回 unsupported", async () => {
    ...
    await expect(core.seek(3)).resolves.toMatchObject({
      status: "unsupported",
    });
    expect(backend.calls).toHaveLength(callCount);
    expect(core.getSnapshot().state).toBe("ready");
  ...
  it("生产代码不引用端平台、网络或具体播放内核", async () => {
    ...
    const contents = await Promise.all(
      productionFiles.map((file) =>
        readFile(fileURLToPath(new URL(file, directory)), "utf8"),
      ),
    );

    expect(contents.join("\n")).not.toMatch(
      /\b(?:DOM|React|fetch|URL|HTMLVideoElement|mpegts|hls)\b/iu,
    );
  });
});
