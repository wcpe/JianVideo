...
): readonly PlaybackQuality[] {
  return qualities.filter(isQuality).map((quality) => ({ ...quality }));
}

export function qualityById(
  qualities: readonly PlaybackQuality[],
  id: string | null,
): PlaybackQuality | null {
  return id === null
    ? null
    : (qualities.find((quality) => quality.id === id) ?? null);
}

export function qualityTarget(
  quality: Pick<PlaybackQuality, "bandwidth" | "height">,
): QualityTarget | null {
...
export function resolveQuality(
  ...
  const candidates = qualities.filter(
    (quality) =>
      validHeight(quality.height) &&
      (maxHeight === null || quality.height <= maxHeight),
  );
  if (candidates.length === 0) return null;
  const sameHeight = candidates.filter(
    (quality) => quality.height === target.height,
  );
  if (sameHeight.length > 0)
    return selectBandwidth(sameHeight, target.bandwidth);
  const lower = candidates.filter(
    (quality) => quality.height !== undefined && quality.height < target.height,
  );
  if (lower.length > 0) return highestQuality(lower);
  return lowestQuality(candidates);
}

export function highestDataSaverQuality(
  qualities: readonly PlaybackQuality[],
): PlaybackQuality | null {
  const candidates = qualities.filter(
    (quality) =>
      validHeight(quality.height) && quality.height <= DATA_SAVER_MAX_HEIGHT,
  );
  return candidates.length === 0 ? null : highestQuality(candidates);
}

export function isPlaybackRate(
  rate: number,
): rate is (typeof PLAYBACK_RATES)[number] {
...
export function validLoopRange(
  a: number,
  b: number,
  duration: number,
): boolean {
  return (
    validLoopPoint(a, duration) &&
    validLoopPoint(b, duration) &&
    b - a >= AB_LOOP_MIN_DURATION
  );
}
...
function selectBandwidth(
  ...
  const notHigher = qualities.filter(
    (quality) =>
      validBandwidth(quality.bandwidth) && quality.bandwidth <= bandwidth,
  );
