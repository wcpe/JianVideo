...
function frame(index: number, overrides: Partial<PresentedFrame> = {}): PresentedFrame {
  return {
...
function target(index: number, overrides: Partial<AdjacentFrameTarget> = {}): AdjacentFrameTarget {
  return {
...
function stableTarget(index: number, stableFrameId: string): AdjacentFrameTarget {
  return { frameDuration: FRAME_DURATION, mediaTime: index * FRAME_DURATION, stableFrameId };
}
...
class TrackingBackend extends FakePlaybackBackend {
  override async seek(request: Parameters<FakePlaybackBackend['seek']>[0]): Promise<SeekResult> {
    ...
    if (result.status === 'completed') {
      this.setSnapshot({ ...this.getSnapshot(), currentTime: result.confirmedTime, requestId: request.requestId });
    }
    return result;
  }
}
...
class ScriptedFrameFacet implements FramePresentationFacet {
  ...
  constructor(current: PresentedFrame | null, adjacent: AdjacentFrameTarget | null, presentedFrames: PresentedFrame[]) {
...
class SequentialFrameFacet implements FramePresentationFacet {
  ...
  getAdjacentFrameTarget(_current: PresentedFrame, direction: FrameStepDirection): AdjacentFrameTarget {
...
function exactSnapshot(currentTime = 10 * FRAME_DURATION) {
  return createSnapshot({
    capabilities: { ...EMPTY_CAPABILITIES, framePresentation: 'exact', seek: 'available' },
    currentTime,
    duration: 20,
...
async function createFrameCore(facet: FramePresentationFacet, snapshot = exactSnapshot()) {
  const backend = new TrackingBackend();
  backend.setSnapshot(snapshot);
  const core = new PlaybackCore({ backend, facets: { framePresentation: facet } });
...
describe('PlaybackCore 逐帧控制', () => {
  it.each([
  ...
  ])('用 sourceFrameIndex 精确验证 $direction 恰好相邻', async ({ direction, end, start }) => {
    const facet = new ScriptedFrameFacet(frame(start), target(end), [frame(end)]);
    ...
    expect(core.getSnapshot()).toMatchObject({ currentTime: end * FRAME_DURATION, state: 'paused' });
  });
  ...
  it('用 stableFrameId 精确验证最终帧等于方向性相邻目标', async () => {
    ...
    const { core } = await createFrameCore(new ScriptedFrameFacet(start, adjacent, [presented]));
  ...
  it.each([
    ...
    { actual: frame(11, { mediaTime: 14 * FRAME_DURATION }), label: '超出时间容差' },
  ])('$label 最多校正 2 次后受控失败', async ({ actual }) => {
    const facet = new ScriptedFrameFacet(frame(10), target(11), [actual, actual, actual]);
    const { backend, core } = await createFrameCore(facet);

    const result = await core.stepFrame('next');

    expect(result).toMatchObject({ correctionCount: 2, precision: 'exact-verified', status: 'failed' });
  ...
  it('最终呈现帧身份缺失时降级为 approximate，不用 mediaTime 冒充精确', async () => {
    ...
    await expect(core.stepFrame('next')).resolves.toMatchObject({ precision: 'approximate', status: 'completed' });
    expect(facet.waitCount).toBe(1);
  });

  it('相邻目标身份缺失时在执行前降级为 approximate', async () => {
    const adjacent: AdjacentFrameTarget = { frameDuration: FRAME_DURATION, mediaTime: 11 * FRAME_DURATION };
  ...
  it.each([
    ...
    { actual: frame(10, { sampleSource: 'backend' as const }), label: '非 rVFC 同帧' },
    { actual: frame(9, { sampleSource: 'backend' as const }), label: '非 rVFC 反向' },
    { actual: frame(14, { sampleSource: 'backend' as const }), label: '非 rVFC 明显跳错' },
  ...
  it('缺少基本 seek 时仍先暂停并返回 unsupported', async () => {
    const facet = new ScriptedFrameFacet(frame(10), target(11), [frame(11)]);
    const snapshot = createSnapshot({
      capabilities: { ...EMPTY_CAPABILITIES, framePresentation: 'exact', seek: 'unavailable' },
      currentTime: 10 * FRAME_DURATION,
      seekable: [{ end: 20, start: 0 }],
    });
    const { backend, core } = await createFrameCore(facet, snapshot);

    await expect(core.stepFrame('next')).resolves.toMatchObject({ precision: 'unsupported', status: 'unsupported' });
  ...
  it('连续 60 次前进再 60 次后退严格串行并基于上一确认帧', async () => {
    ...
    const { backend, core } = await createFrameCore(facet, exactSnapshot(100 * FRAME_DURATION));

    const forward = await Promise.all(Array.from({ length: 60 }, () => core.stepFrame('next')));
    const backward = await Promise.all(Array.from({ length: 60 }, () => core.stepFrame('previous')));

    expect(forward.map(({ confirmedSourceFrameIndex }) => confirmedSourceFrameIndex)).toEqual(
      Array.from({ length: 60 }, (_, index) => 101 + index),
    );
    expect(backward.map(({ confirmedSourceFrameIndex }) => confirmedSourceFrameIndex)).toEqual(
      Array.from({ length: 60 }, (_, index) => 159 - index),
    );
    expect(facet.current.sourceFrameIndex).toBe(100);
    expect(backend.calls.filter(({ method }) => method === 'seek')).toHaveLength(120);
  });
  ...
  it('ready 态先等待 pause 完成，再发起逐帧 seek', async () => {
    ...
    expect(backend.calls.map(({ method }) => method).slice(-2)).toEqual(['pause', 'seek']);
    expect(core.getSnapshot().state).toBe('paused');
  });
  ...
  it('paused 态直接逐帧，不重复调用 pause', async () => {
    ...
    const { backend, core } = await createFrameCore(facet);
    await core.pause();
    const pauseCount = backend.calls.filter(({ method }) => method === 'pause').length;

    await expect(core.stepFrame('next')).resolves.toMatchObject({ status: 'completed' });

    expect(backend.calls.filter(({ method }) => method === 'pause')).toHaveLength(pauseCount);
    expect(core.getSnapshot().state).toBe('paused');
  });

  it.each([
    { direction: 'previous' as const, expectedTime: 599 * FRAME_DURATION, seekCount: 1 },
    { direction: 'next' as const, expectedTime: 20, seekCount: 0 },
  ])('ended 态 $direction 先暂停确认并正确处理片尾边界', async ({ direction, expectedTime, seekCount }) => {
    ...
    pendingPause.resolve();
    await expect(stepping).resolves.toMatchObject({ confirmedMediaTime: expectedTime, status: 'completed' });
    expect(backend.calls.filter(({ method }) => method === 'seek')).toHaveLength(seekCount);
    expect(core.getSnapshot()).toMatchObject({ currentTime: expectedTime, state: 'paused' });
  });
  ...
  it('loading 态受控返回 unsupported 且不发起 seek', async () => {
    ...
    await expect(core.stepFrame('next')).resolves.toMatchObject({ status: 'unsupported' });
    expect(backend.calls.filter(({ method }) => method === 'seek')).toHaveLength(0);
  ...
  it('seeking 态受控返回 unsupported 且不发起第二次 seek', async () => {
    ...
    backend.seekHandler = (request) => {
      seekCount += 1;
      return seekCount === 1 ? pendingSeek.promise : Promise.resolve({
        clamped: false,
    ...
    await expect(core.stepFrame('next')).resolves.toMatchObject({ status: 'unsupported' });
    expect(backend.calls.filter(({ method }) => method === 'seek')).toHaveLength(1);

    pendingSeek.resolve({ clamped: false, confirmedTime: 8, requestId: 2, requestedTime: 8, status: 'completed', targetTime: 8 });
    await seeking;
  });
  ...
  it('idle 与 disposed 态分别受控返回 unsupported 和 canceled', async () => {
    ...
    const idleCore = new PlaybackCore({ backend: idleBackend, facets: { framePresentation: facet } });
    await expect(idleCore.stepFrame('next')).resolves.toMatchObject({ status: 'unsupported' });
    expect(idleBackend.calls).toHaveLength(0);

    const { backend, core } = await createFrameCore(facet);
    core.dispose();
    const callCount = backend.calls.length;
    await expect(core.stepFrame('next')).resolves.toMatchObject({ status: 'canceled' });
  ...
  it('切源安全取代等待中的逐帧，旧呈现结果不回写', async () => {
    ...
    await Promise.resolve();
    expect(core.getSnapshot()).toMatchObject({ error: null, sourceId: SOURCE_B.id });
  });
  ...
  it('逐帧事件顶层 requestId 来自完整结果，getter 持续暴露上次逐帧精度', async () => {
    const facet = new ScriptedFrameFacet(frame(10), target(11), [frame(11)]);
    const { core } = await createFrameCore(facet);
    const events: Array<{ readonly requestId: number; readonly resultRequestId: number; readonly type: string }> = [];
    core.subscribe((event) => {
      if (event.type === 'frameStepCompleted' || event.type === 'commandCompleted') {
        events.push({ requestId: event.requestId, resultRequestId: event.result.requestId, type: event.type });
      }
    });

    const result = await core.stepFrame('next');

    const expected = { requestId: result.requestId, resultRequestId: result.requestId };
