...
const COMMAND_A: PlaybackCommandContext = { requestId: 10, sourceEpoch: 1, sourceId: 'source-a' };

function cue(startTime: number, endTime: number, assetId = `sprite-${String(startTime)}`): PreparedPreviewCue {
  return {
    endTime,
    sprite: { assetId, height: 90, width: 160, x: startTime + 1, y: startTime + 2 },
    startTime,
  };
}
...
describe('PreviewFacet', () => {
  ...
  it('命中首尾 cue、缝隙与半开区间边界', () => {
    ...
    expect(facet.hitTest(0, COMMAND_A)).toMatchObject({ generationId: 'generation-a', startTime: 0 });
    ...
    expect(facet.hitTest(4.999, COMMAND_A)).toMatchObject({ endTime: 5, profileId: 'profile-a' });
  ...
  it('generation 与 profile 切换后只返回当前轨道元数据', () => {
    ...
    facet.setTrack(
      track([cue(0, 2)], { generationId: 'generation-b', profileId: 'profile-b' }),
  ...
  it('旧 sourceEpoch 与同代次旧 requestId 不得覆盖当前轨道', () => {
    const facet = createPreviewFacet();
    const currentCommand = { requestId: 20, sourceEpoch: 2, sourceId: 'source-b' };
  ...
  it('旧命令在轨道校验前忽略，非法迟到响应也不抛出', () => {
    ...
    expect(() =>
      facet.setTrack(invalidTrack, { requestId: 99, sourceEpoch: 1, sourceId: 'source-a' }),
    ).not.toThrow();
    expect(() => facet.setTrack(invalidTrack, { ...currentCommand, requestId: 19 })).not.toThrow();
    expect(() =>
      facet.setTrack(invalidTrack, { ...currentCommand, requestId: 99, sourceId: 'source-c' }),
    ).not.toThrow();
    expect(facet.hitTest(1, currentCommand)?.sprite.assetId).toBe('current');
  });
  ...
  it.each([
    ...
    ['cue 终点非有限', track([{ ...cue(0, 2), endTime: Number.POSITIVE_INFINITY }])],
    ['cue 起点为负', track([cue(-1, 2)])],
    ['cue 终点不大于起点', track([cue(2, 2)])],
    ['cue 顺序倒退', track([cue(3, 4), cue(1, 2)])],
    ['cue 相互重叠', track([cue(0, 3), cue(2, 4)])],
    ['sprite 横坐标为负', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, x: -1 } }])],
    ['sprite 纵坐标为负', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, y: -1 } }])],
    ['sprite 宽度非正', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, width: 0 } }])],
    ['sprite 高度非正', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, height: 0 } }])],
    ['sprite 高度非有限', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, height: Number.NaN } }])],
    ...
    ['track sourceFingerprint 为空', track([cue(0, 2)], { sourceFingerprint: '' })],
    ['sprite assetId 为空', track([{ ...cue(0, 2), sprite: { ...cue(0, 2).sprite, assetId: '' } }])],
  ])('拒绝%s且不污染现有状态', (_label, invalidTrack) => {
    const facet = createPreviewFacet();
    facet.setTrack(track([cue(0, 2, 'valid')]), COMMAND_A);
    const before = facet.getState();

    expect(() => facet.setTrack(invalidTrack, { ...COMMAND_A, requestId: 11 })).toThrow(Error);
  ...
  it('复制已准备轨道，调用方后续修改不会污染命中结果', () => {
    ...
    const mutableCue = cue(0, 2, 'original') as {
      endTime: number;
      sprite: { assetId: string; height: number; width: number; x: number; y: number };
      startTime: number;
    };
    ...
    expect(facet.hitTest(1, COMMAND_A)).toMatchObject({ endTime: 2, sprite: { assetId: 'original' } });
    expect(facet.hitTest(50, COMMAND_A)).toBeNull();
  });

  it('生产实现不引用网络、DOM、地址或图片 API', async () => {
    const source = await readFile(fileURLToPath(new URL('preview-facet.ts', import.meta.url)), 'utf8');

    expect(source).not.toMatch(/\b(?:fetch|XMLHttpRequest|document|window|Image|URL|HTMLImageElement)\b/u);
  });
});
