...
const AUDIO_B: PlaybackTrack = { ...AUDIO_A, default: false, id: 'audio-b', label: 'English', language: 'en' };
...
class FakeTrackFacet implements TrackFacet {
  readonly calls: Array<{ readonly command: PlaybackCommandContext; readonly kind: TrackKind; readonly trackId: string | null }> = [];
  readonly tracks: Record<TrackKind, readonly PlaybackTrack[]> = {
    audio: [AUDIO_A, AUDIO_B],
    subtitle: [SUBTITLE_A],
  };
  selectHandler: ((kind: TrackKind, trackId: string | null, command: PlaybackCommandContext) => Promise<void>) | undefined;
  ...
  confirmState(kind: TrackKind, trackId: string | null, command: PlaybackCommandContext): void {
    this.setState(kind, trackId, command.sourceId, command.sourceEpoch, command.requestId);
  }

  setState(kind: TrackKind, trackId: string | null, sourceId = SOURCE_A.id, sourceEpoch = 1, requestId = 0): void {
    this.states[kind] = {
...
function selectionState(kind: TrackKind, trackId: string | null): TrackSelectionState {
  return {
    effectiveTrackId: trackId,
    kind,
    requestId: 0,
...
function omitTrackField(track: PlaybackTrack, field: 'available' | 'capability'): PlaybackTrack {
  if (field === 'available') {
...
async function createLoadedCore(facet?: TrackFacet): Promise<{ backend: FakePlaybackBackend; core: PlaybackCore }> {
  const backend = new FakePlaybackBackend();
  backend.setSnapshot(
    createSnapshot({ capabilities: { ...EMPTY_CAPABILITIES, tracks: 'available' }, sourceId: SOURCE_A.id, state: 'ready' }),
  );
  const binding = facet === undefined ? { backend } : { backend, facets: { tracks: facet } };
...
function trackEvents(events: readonly PlaybackEvent[]): PlaybackEvent[] {
  return events.filter((event) => event.type === 'trackSelectionChanged' || event.type === 'trackSelectionCompleted');
}
...
describe('PlaybackCore TrackFacet', () => {
  ...
  it('能力快照保持后端真值，不根据轨道列表数量推断', async () => {
    ...
    const backend = new FakePlaybackBackend();
    backend.setSnapshot(
      createSnapshot({ capabilities: EMPTY_CAPABILITIES, sourceId: SOURCE_A.id, state: 'ready' }),
    );
    const unavailable = new PlaybackCore({ backend, facets: { tracks: new FakeTrackFacet() } });
  ...
  it('能力快照缺少 tracks 时按 unavailable 处理', async () => {
    ...
    const backend = new FakePlaybackBackend();
    backend.setSnapshot(
      createSnapshot({ capabilities: capabilitiesWithoutTracks(), sourceId: SOURCE_A.id, state: 'ready' }),
    );
    const core = new PlaybackCore({ backend, facets: { tracks: facet } });
    await core.load(SOURCE_A);

    expect(core.getSnapshot().capabilities.tracks).toBe('unavailable');
    await expect(core.selectTrack('audio', AUDIO_B.id)).resolves.toMatchObject({ status: 'unsupported' });
  ...
  it('字幕关闭沿用统一选择路径并以 null 确认生效', async () => {
    ...
    expect(core.getTrackSelection('subtitle')).toMatchObject({ effectiveTrackId: null, requestId: 2, selectedTrackId: null });
  });
  ...
  it('同步 throw 前不发布临时 selected 状态', async () => {
    ...
    expect(trackEvents(events)).not.toContainEqual(
      expect.objectContaining({ selectedTrackId: AUDIO_B.id, type: 'trackSelectionChanged' }),
    );
  });
  ...
  it('新请求同步 throw 仍立即取代旧请求，旧确认迟到不污染缓存', async () => {
    ...
    await expect(firstSelection).resolves.toMatchObject({ requestId: 2, status: 'superseded' });
    ...
    expect(events).not.toContainEqual(
      expect.objectContaining({ requestId: 3, selectedTrackId: AUDIO_B.id, type: 'trackSelectionChanged' }),
    );
  ...
  it.each([
    ['同步 throw', () => { throw new PlaybackBackendError('media', '同步切换失败'); }],
    ['Promise reject', () => Promise.reject(new PlaybackBackendError('media', '异步切换失败'))],
  ...
  it('后请求完成后忽略旧确认迟到，并为每个请求保留准确完成快照', async () => {
    ...
    const completed = events.filter(
      (event): event is Extract<PlaybackEvent, { readonly type: 'trackSelectionCompleted' }> =>
    ...
    expect(firstCompleted).toMatchObject({
      result: { requestId: 2, selectedTrackId: AUDIO_B.id, status: 'superseded' },
    ...
    expect(secondCompleted).toMatchObject({
      result: { requestId: 3, selectedTrackId: AUDIO_A.id, status: 'completed' },
  ...
  it('getter 每次同步读取 facet 外部变化', async () => {
    ...
    expect(core.getTrackSelection('audio')).toMatchObject({ effectiveTrackId: AUDIO_A.id });
  ...
  it('无代次 state 仅作初始值，选择历史后需更高代次才接受外部变化', async () => {
    ...
    facet.selectHandler = (kind, trackId, command) => {
      state = { ...selectionState(kind, trackId), requestId: command.requestId };
      return Promise.resolve();
    };
    const { core } = await createLoadedCore(facet);

    expect(core.getTrackSelection('audio')).toMatchObject({ effectiveTrackId: AUDIO_A.id, requestId: 0 });
    await expect(core.selectTrack('audio', AUDIO_B.id)).resolves.toMatchObject({ requestId: 2, status: 'completed' });

    state = initial;
    expect(core.getTrackSelection('audio')).toMatchObject({ effectiveTrackId: AUDIO_B.id, requestId: 2 });
    state = { ...selectionState('audio', AUDIO_A.id), requestId: 3 };
    expect(core.getTrackSelection('audio')).toMatchObject({ effectiveTrackId: AUDIO_A.id, requestId: 3 });
  });
  ...
  it.each([
    ...
    },
    {
      ...
      state: { ...selectionState('audio', AUDIO_A.id), effectiveTrackId: false },
    },
    {
    ...
    },
    {
      ...
      state: { ...selectionState('audio', AUDIO_A.id), sourceEpoch: Number.NaN },
    },
    {
      name: 'sourceId 类型非法',
    ...
    },
    {
      ...
      state: { ...selectionState('audio', AUDIO_A.id), selectedTrackId: 'missing' },
    },
    {
      name: 'effectiveTrackId 不存在',
      state: { ...selectionState('audio', AUDIO_A.id), effectiveTrackId: 'missing' },
    },
    ...
    await expect(core.selectTrack('audio', AUDIO_B.id)).resolves.toMatchObject({ status: 'failed' });
    expect(facet.calls).toEqual([]);
  });
  ...
  ])('允许音轨 selected/effective 独立为空且 getter 保持后端真值', async (state) => {
    const facet = new FakeTrackFacet();
    facet.stateHandler = () => ({ ...selectionState('audio', AUDIO_A.id), ...state });
    const { core } = await createLoadedCore(facet);
  ...
  it('允许切换期间 selected 指向可用目标且 effective 保持实际轨道', async () => {
    const facet = new FakeTrackFacet();
    facet.stateHandler = () => ({ ...selectionState('audio', AUDIO_A.id), selectedTrackId: AUDIO_B.id });
    const { core } = await createLoadedCore(facet);
  ...
  it('当前 effective 音轨不可切仍是合法 state，选择另一不可切目标受控 unsupported', async () => {
    const facet = new FakeTrackFacet();
    const current = { ...AUDIO_A, capability: 'unsupported' as const, unsupportedReason: '当前音轨不可切换' };
    const target = { ...AUDIO_B, capability: 'unsupported' as const, unsupportedReason: '目标音轨不可切换' };
  ...
  it('严格拒绝不可用轨道出现在选择 state', async () => {
    ...
    expect(core.getTrackSelection('audio')).toBeNull();
    await expect(core.selectTrack('audio', AUDIO_A.id)).resolves.toMatchObject({ status: 'failed' });
    expect(facet.calls).toEqual([]);
  });
  ...
  it('非法 kind 不发布轨道公共事件', async () => {
    ...
    await expect(core.selectTrack('video' as TrackKind, 'video-a')).resolves.toMatchObject({ status: 'unsupported' });
  ...
  it('切源隔离旧选择结果并按新源重新读取选择状态', async () => {
    ...
    await expect(selection).resolves.toMatchObject({ requestId: 2, status: 'superseded' });
    expect(core.getTrackSelection('audio')).toMatchObject({
      effectiveTrackId: AUDIO_B.id,
      selectedTrackId: AUDIO_B.id,
      sourceEpoch: 2,
  ...
  it('dispose 取消待处理选择且旧结果不得覆盖终态', async () => {
    ...
    await expect(selection).resolves.toMatchObject({ requestId: 2, status: 'canceled' });
    pending.resolve();
    await Promise.resolve();
  ...
  it('无 facet、能力不可用、非法 kind/track 和轨道不支持均受控 unsupported 且不调用后端', async () => {
    ...
    await expect(withoutFacet.core.selectTrack('audio', AUDIO_A.id)).resolves.toMatchObject({ status: 'unsupported' });

    const facet = new FakeTrackFacet();
    const backend = new FakePlaybackBackend();
    backend.setSnapshot(createSnapshot({ capabilities: EMPTY_CAPABILITIES, sourceId: SOURCE_A.id, state: 'ready' }));
    const unavailable = new PlaybackCore({ backend, facets: { tracks: facet } });
    await unavailable.load(SOURCE_A);
    await expect(unavailable.selectTrack('audio', AUDIO_A.id)).resolves.toMatchObject({ status: 'unsupported' });

    const available = await createLoadedCore(facet);
    await expect(available.core.selectTrack('video' as TrackKind, 'video-a')).resolves.toMatchObject({ status: 'unsupported' });
    await expect(available.core.selectTrack('audio', 'missing')).resolves.toMatchObject({ status: 'unsupported' });
    await expect(available.core.selectTrack('audio', null)).resolves.toMatchObject({ status: 'unsupported' });

    const unsupportedTrack = { ...AUDIO_B, capability: 'unsupported' as const, unsupportedReason: '当前路径不可切换' };
    facet.tracks.audio = [AUDIO_A, unsupportedTrack];
    await expect(available.core.selectTrack('audio', AUDIO_B.id)).resolves.toMatchObject({
  ...
  it('选择事件顶层 requestId 与源身份一致，完成事件携带状态和错误', async () => {
    const facet = new FakeTrackFacet();
    facet.selectHandler = () => Promise.reject(new PlaybackBackendError('unsupported', '后端不支持切换'));
    const { core } = await createLoadedCore(facet);
    const events: PlaybackEvent[] = [];
    core.subscribe((event) => events.push(event));
    ...
    const selectionEvents = trackEvents(events) as Array<
      Extract<PlaybackEvent, { readonly type: 'trackSelectionChanged' | 'trackSelectionCompleted' }>
    >;
