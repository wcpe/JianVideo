...
describe("render-pixi package", () => {
  ...
  it("HLS 预览只在 hover 或选中时触发", () => {
    expect(shouldRequestHlsPreview({ hovered: false, selected: false })).toBe(
      false,
    );
    expect(shouldRequestHlsPreview({ hovered: true, selected: false })).toBe(
      true,
    );
    expect(shouldRequestHlsPreview({ hovered: false, selected: true })).toBe(
      true,
    );
  });
  ...
  it("挂载 Pixi 预览并暴露可销毁句柄", async () => {
    ...
    const handle = await mountPixiGridPreview({
      columns: 2,
      height: 120,
      host,
      rows: 1,
