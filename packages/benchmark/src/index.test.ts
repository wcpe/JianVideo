...
describe("benchmark package", () => {
  ...
  it("超出 FR2-003 前端门时给出中文原因", () => {
    ...
    expect(report.frontend.failures).toContain(
      "连续滚动帧耗时 p95 超过 16.7ms",
    );
    expect(report.frontend.failures).toContain(
      "10s 滚动窗口存在超过 200ms 的长任务",
    );
  ...
  it("把 benchmark summary 写入可复跑目录", async () => {
    ...
    const result = await writeBenchmarkSummary(
      outputRoot,
      {
        backend: [
          evaluateBackendQueryBenchmark({
        ...
        coverageNotes: [
          "真实 PixiJS 预览画布、Canvas 非空检查与 HLS 预览计数通过",
        ],
