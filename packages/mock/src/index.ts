...
export function handleMockApiRequest(
  ...
  if (method === "GET" && url.pathname.startsWith("/api/v2/media/")) {
    return mediaDetailResponse(
      url.pathname.slice("/api/v2/media/".length),
      spaceId,
    );
  }
  ...
  if (method === "GET" && url.pathname.startsWith("/api/v2/tasks/")) {
    return taskDetailResponse(
      url.pathname.slice("/api/v2/tasks/".length),
      spaceId,
      taskReads,
      taskOverrides,
    );
...
function mediaDetailResponse(id: string, spaceId: string): Response {
  const item = mediaFixtures.find(
    (media) => media.id === id && media.space_id === spaceId,
  );
  if (item === undefined) {
...
function visibleTasks(
  taskOverrides: Map<string, MockTaskItem>,
): readonly MockTaskItem[] {
  return taskFixtures.map((task) => taskOverrides.get(task.id) ?? task);
}

function updateTask(
  task: MockTaskItem,
  patch: Partial<MockTaskItem>,
): MockTaskItem {
  return { ...task, ...patch, updated_at: "2026-07-01T10:02:00Z" };
}

function normalizeMockTaskStatus(
  status: MockTaskStatus,
): Exclude<MockTaskStatus, "completed" | "error"> {
  if (status === "completed") {
    return "succeeded";
  }
  if (status === "error") {
...
function getRequestLike(
  input: RequestInfo | URL,
): Pick<Request, "headers" | "method" | "url"> | undefined {
...
export function createMockMediaIndex(
  options: MockMediaIndexOptions,
): MockMediaIndex {
  ...
  return {
    ...
    queryWindow(query: MockMediaWindowQuery) {
      ...
      for (
        let position = 0;
        position < total && items.length < limit;
        position += 1
      ) {
...
function matchesQuery(
  record: MockMediaRecord,
  query: MockMediaWindowQuery,
): boolean {
  ...
  if (
    ...
    !record.path.startsWith(query.pathPrefix)
  ) {
    return false;
  }
  return true;
