...
describe("mock package", () => {
  ...
  it("覆盖 wiki 必需的 mock 场景与状态面", () => {
    ...
    const surfaces = new Set(
      mockScenarios.flatMap((scenario) => scenario.surfaces),
    );
  ...
  it("未知场景抛出中文错误", () => {
    expect(() => findScenario("unknown-scenario" as never)).toThrow(
      "未知 mock 场景",
    );
  });
  ...
  it("按 Space 返回媒体分页", async () => {
    ...
    const response = await fetch(
      "https://mock.local/api/v2/media?page=1&page_size=1",
      {
        headers: { "X-JianVideo-Space-Id": "space-studio" },
      },
    );
    const body = (await response.json()) as {
      readonly items: readonly [{ readonly space_id: string }];
  ...
  it("任务轮询从 running 进入 succeeded", async () => {
    ...
    const first = await fetch(
      "https://mock.local/api/v2/tasks/task-transcode-default",
      {
  ...
  it("通用任务列表支持 Space、类型和状态过滤", async () => {
    const response = await createMockFetch()(
      "https://mock.local/api/tasks?type=transcode&status=failed",
      {
        headers: { "X-JianVideo-Space-Id": "space-default" },
      },
    );
  ...
  it("通用任务统计按 Space 聚合并映射状态桶", async () => {
    const response = await createMockFetch()(
      "https://mock.local/api/tasks/stats",
      {
        headers: { "X-JianVideo-Space-Id": "space-default" },
      },
    );
  ...
  it("通用任务支持取消和重试", async () => {
    ...
    const canceled = await fetch(
      "https://mock.local/api/tasks/task-transcode-default/cancel",
      {
        method: "POST",
        headers: { "X-JianVideo-Space-Id": "space-default" },
      },
    ...
    const retried = await fetch(
      "https://mock.local/api/tasks/task-transcode-failed/retry",
      {
        method: "POST",
        headers: { "X-JianVideo-Space-Id": "space-default" },
      },
  ...
  it("默认分页参数可回退", async () => {
    const response = await createMockFetch()(
      new URL("https://mock.local/api/v2/media"),
      {
        headers: { "X-JianVideo-Space-Id": "space-default" },
      },
    );
  ...
  it("未知任务和未知路径返回中文错误", async () => {
    ...
    const missingPath = handleMockApiRequest(
      new Request("https://mock.local/api/v2/unknown"),
    );
    const taskBody = (await missingTask.json()) as {
      readonly code: string;
      readonly message: string;
    };
    const pathBody = (await missingPath.json()) as {
      readonly code: string;
      readonly message: string;
    };
  ...
  it("同一 seed 生成稳定媒体条目", () => {
    const first = createMockMediaIndex({
      dataset: "media-index-1m",
      seed: "fr2-063",
    });
    const second = createMockMediaIndex({
      dataset: "media-index-1m",
      seed: "fr2-063",
    });
    const other = createMockMediaIndex({
      dataset: "media-index-1m",
      seed: "other-seed",
    });
  ...
  it("窗口查询不常驻百万完整对象", () => {
    const index = createMockMediaIndex({
      dataset: "media-index-1m",
      seed: "fr2-063",
    });
  ...
  it("索引数据集覆盖 1m、5m 与 10m 三档", () => {
    expect(
      createMockMediaIndex({ dataset: "media-index-1m", seed: "fr2-063" })
        .total,
    ).toBe(1_000_000);
    expect(
      createMockMediaIndex({ dataset: "media-index-5m", seed: "fr2-063" })
        .total,
    ).toBe(5_000_000);
    expect(
      createMockMediaIndex({ dataset: "media-index-10m", seed: "fr2-063" })
        .total,
    ).toBe(10_000_000);
  });
});
