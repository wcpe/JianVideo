...
- **FR2-012 全量 stub 能力**：`SeedStubFixture` 扩展至全部 5 种 task_type（OCR/embedding/object_scene/face/video_understanding）；stub `Infer` 按类型产出结构化 payload（对象/场景标签、人脸框、视频分段摘要）。
- **FR2-012 后置（语义搜索入口）**：时间轴页「语义搜索」开关，复用 `/api/ai/search` 命中 media；AI 关闭时提示不阻断。
- **FR2-012 后置（审核列表与批量）**：`GET /api/ai/results` 放宽为 Space 级列表（可选 `task_type`/`manual` 过滤）；`POST /api/ai/results/batch/confirm|reject`；独立审核列表页 `/ai-review`，支持批量勾选与操作。
...
- **发布门修复**：`dev` 上恰好位于稳定 tag 时实验构建成功跳过（不再 exit 1 刷红）；正式/RC tag 校验提交必须落在 `origin/main` 历史（禁止只在 `dev` 打正式 tag）。
