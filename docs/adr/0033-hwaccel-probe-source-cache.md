...
- 实测结果按 **FFmpeg 可执行文件身份摘要持久化到 SQLite**（新表 `codec_probe_caches`，身份包含版本、实际路径与文件元数据）。同一可执行文件复用缓存、避免重复 3 分钟实测；版本、路径或文件发生变化即视为失效、下次重测；提供**手动「重新测试」**强制重跑。
...
- **持久化按可执行文件身份失效**：版本号不足以区分同版本的不同编译配置；将版本、实际路径与文件元数据共同纳入身份，既能跨重启复用，又能在升级、换路径或原地替换 FFmpeg 后自动重测。
