...
> 状态：已审核接受　·　关联 PRD：FR2-031　·　阶段：P2 `0.23.x`　·　分支：待定
...
- [ ] 定义推断 schema 与规则输出结构。
- [ ] 实现 movie/series/mixed/home_video 解析规则。
- [ ] 接入扫描与批量 backfill 任务。
- [ ] 新增查询、人工纠正、关闭配置 API。
- [ ] 前端详情或编辑入口展示和修正推断结果。
- [ ] 接入审计事件。
- [ ] 补单元测试：中英文季集、年份、目录回退、低置信、关闭开关、展示优先级、不覆盖人工值。
- [ ] 补集成测试：扫描后推断、关闭后不推断、backfill 重跑。
- [ ] 补 E2E：详情页修正推断并刷新保留。
