...
> 状态：已实现并完成终审 P1 修复　·　关联 PRD：FR2-031　·　阶段：P2 `0.23.x`　·　复核分支：`feature/p2-fr2-031-complete`
...
- 推断设置真实变化时原子递增任务代次；初始代次键缺失或解析为 `0` 时，运行中的旧任务仍必须在批次边界比较当前代次并停止，非法负代次不得绕过比较；重新开启或扩大范围会创建新代次补偿任务。
...
- 终审 P1 回归：`go test ./internal/library -run 'TestBackfillGenerationZeroStopsAfterSettingsChangeAndNewGenerationCompensates|TestInferenceGenerationChangedComparesNonPositiveExpectedValues' -count=1` 修复前稳定失败，旧任务在初始代次为 `0` 时写入了第二批（`updated=101`），且非法负代次绕过比较；修复后通过，验证全局/按库关闭均在下一批边界停止，并由新代次补偿剩余缺失项。
