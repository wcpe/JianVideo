...
- 时间轴提供全局“全部 / 已推断 / 未推断”筛选，并在媒体卡片展示推断标题。
...
- 扫描入库时同步推断；手工批量重推断走 FR2-037 `full` 任务，设置变更走 `missing` 增量任务，只处理尚无推断记录的媒体。
- 人工纠正写审计事件，并设置 `manual=true`；后续自动推断不得覆盖。
- 媒体列表通过 `inference=auto/manual/missing` 服务端筛选推断状态，避免仅过滤当前分页结果。
...
- 时间轴可在全局媒体范围筛选已推断/未推断状态，筛选请求由服务端执行。
