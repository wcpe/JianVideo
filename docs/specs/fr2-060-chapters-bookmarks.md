...
> 状态：候选发布@v0.24.0-rc.1　·　关联 PRD：FR2-060　·　阶段：P3 `0.24.x`　·　分支：待定
...
- [x] 新增章节与书签 schema、约束、索引和版本化迁移。
- [x] 扩展 ffprobe 规范化解析，支持真实内嵌章节、合法性校验和事务整组刷新。
- [x] 将章节提取接入扫描刷新和 metadata backfill。
- [x] 实现只读章节查询 API，不注册章节写端点。
- [x] 实现书签列表、创建、编辑、删除 API、递增 `revision` 并发冲突和 Space 隔离。
- [x] 接入书签审计事件，不复制完整备注。
- [x] 由 `media-client` 承担章节/书签请求、缓存失效和冲突处理，player-core 只维护端无关 `Chapter` / `Bookmark` 控制状态与 seek 意图。
- [x] 播放器展示章节边界、章节列表、书签标记和书签 CRUD。
- [x] 验证媒体软删期间章节/书签保留且普通查询不可见；物理清理级联留给 FR2-054/P5。
- [x] 提交真实内嵌章节媒体 fixture 及其来源、生成方式、版本和 SHA-256 说明。
- [x] 补单元测试：章节时间换算/校验/标题回退、书签文本与时间校验、冲突判断。
- [x] 补集成测试：真实 ffprobe fixture、迁移、backfill 幂等、刷新失败保留旧章节、Space 隔离、审计。
- [x] 补 E2E：章节跳转、书签创建/编辑/删除、两个浏览器上下文跨端刷新同步、软删不可见和无截图请求。
- [x] 完成 Windows 已安装 PWA 的真实媒体 headed 验收，覆盖章节展示/跳转、书签 CRUD、刷新后持久化和冲突重拉。
- [x] 文档同步：实现完成后更新 PRD 状态、ARCHITECTURE、API 和 CHANGELOG。
