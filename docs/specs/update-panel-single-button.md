...
- 范围内：
  ...
  - 切换频道（正式版/候选版 RC）时同样只回填该频道缓存、不自动联网。
...
- 候选版 RC 缓存读取时校验 `tag/latest` 均严格匹配 `vX.Y.Z-rc.N`（`N>=1`）且 `prerelease=true`；旧 `dev` 缓存迁移时自动清除并按无缓存处理，正式版缓存保持兼容。
...
- 无缓存进入：不显示版本/更新区。
- 有缓存进入：直接显示缓存版本，且**不发起网络请求**（断言 check 端点未被调用）；候选版 RC 的旧 `dev` 或非法 RC 缓存先清除并按无缓存处理。
