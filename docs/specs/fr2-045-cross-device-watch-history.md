...
> 状态：候选发布@v0.24.0-rc.1　·　关联 PRD：FR2-045　·　阶段：P3 `0.24.x`
...
- [x] 在 `media-client` / `player-core` 接入读取、单请求合并、冲突采用与前后台补报。
- [x] 补迁移、并发、当前会话乱序/重复、历史会话迟到、`ab_loop` 不完成、跨 Space 隔离和 P5 回填预演测试。
- [x] 使用真实媒体完成独立浏览器上下文与安装态 PWA 上下文的跨端续播验收。
