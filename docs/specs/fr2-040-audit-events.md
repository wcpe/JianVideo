...
> 状态：已交付@v0.23.0　·　关联 PRD：FR2-040　·　阶段：P2 `0.23.x`　·　分支：codex/fr2-040-audit-events
...
- [x] 新增 `audit_events` model、repository、脱敏工具与服务接口。
- [x] 实现审计查询 API 与 cursor 分页。
- [x] 接入配置变更、库变更、媒体移动/删除/重命名/还原、元数据回写、任务操作、缓存清理、迁移事件。
- [x] 前端增加审计事件列表与详情展示。
- [x] 补单元测试：脱敏、事件输入校验、cursor 分页。
- [x] 补集成测试：关键 API 成功后写事件，失败请求不写成功事件，跨 Space 隔离。
- [x] 补 E2E：执行设置保存或媒体操作后能查到审计事件。
