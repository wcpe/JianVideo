...
- 移动触摸手势：
  ...
  - 非全屏页面中的纵向页面滚动优先于播放器手势；只有明确命中右半区音量手势并完成方向锁定后才阻止默认滚动。全屏或 standalone PWA 可在播放器区域内完整接管已锁定手势。
  - 触摸手势采用 Pointer Events 或等价统一输入层；鼠标拖动不自动解释为移动端滑动手势。
- 亮度能力降级：
  - Web/PWA 不展示可操作的“系统亮度”滑动手势或设置项。
  - 左半区纵向滑动不改变视频 CSS `filter`、页面遮罩、主题亮度或任何仅影响站内画面的值来冒充系统屏幕亮度。
  - 能力说明必须明确写为“浏览器不支持调节系统亮度”；不把该项记为成功，也不发送模拟亮度埋点。
  - 若未来浏览器提供经过验证的标准亮度 API，须另行更新规格与兼容矩阵后才能启用。
...
- `systemBrightness`：P3 Web/PWA 固定为 `unsupported`。
...
- 未超过方向锁定阈值前不改变媒体；超过阈值后按主轴锁定：横向为 seek，右半区纵向为音量，其余情况取消并交还浏览器默认行为。
- seek 偏移按播放器宽度映射，并设置固定最大跨度，避免短视频过敏或长视频一次滑到结尾；映射函数作为纯函数测试，最终值夹取到 `[0, duration]`。
- 音量按播放器高度映射，移动中可预览并实时应用；取消时恢复手势开始前音量，正常结束时保留最终值。
- 多指触摸、第二 pointer 加入、浏览器 `pointercancel`、失去捕获或播放器卸载时安全取消。
- 视觉提示使用播放器现有 overlay 层，不遮挡系统 PiP，不宣称改变系统音量；这里调整的是当前媒体元素音量。

### 3.5 证据分层

| 层级 | 环境 | 可证明内容 | 不能替代 |
|---|---|---|---|
| L1 单元/组件自动化 | Vitest/jsdom，浏览器 API mock | 能力探测、状态机、action handler、手势方向锁定、边界夹取、取消清理、亮度始终 unsupported | 真实 PiP 窗口、系统媒体面板、后台调度、真实触摸手感 |
| L2 浏览器自动化 | Playwright Chromium，含触摸设备仿真与 headed 用例 | UI 可见性、事件接线、横滑 seek、右侧纵滑音量、控件热区避让、前后台状态恢复 | 已安装 PWA、操作系统级 PiP/媒体键、锁屏与省电策略 |
...
- [ ] 在 `WebPlatformAdapter` 或 `frontend` 壳层建立 PiP、Media Session、触摸手势和系统亮度的独立能力模型，不扩展 `PlaybackBackend`。
- [ ] 定义映射给 player-core 的端无关能力状态与命令，静态禁止 DOM/浏览器类型进入核心契约。
- [ ] 接入原生 PiP 状态机、错误提示和资源清理。
- [ ] 接入 Media Session 元数据、action handler、播放状态和位置同步。
- [ ] 实现横向 seek、右半区纵向媒体音量手势及控件热区避让。
- [ ] 明确隐藏 Web 系统亮度入口，补“不支持系统亮度”的能力说明。
- [ ] 补单元/组件测试：能力探测、PiP 状态机、Media Session handler、手势纯函数、取消和边界。
- [ ] 补 Playwright：触摸仿真、前后台切换、能力降级和不出现伪亮度 UI。
...
- 控件热区、多指、短距离移动、`pointercancel`、普通页面纵向滚动均不会误触 seek 或音量。
- Web/PWA 能力模型中 `systemBrightness=unsupported`；产品 UI 和测试快照中不存在可操作亮度手势、CSS 伪亮度或虚假成功提示。
...
- 使用 Playwright headed 的触摸仿真完成横滑 seek、右侧纵滑音量、页面滚动避让；若测试机具备真实触摸屏，再追加真实触摸手感记录，但不以 DevTools/jsdom 结果替代 PiP/后台真机项。
...
- 已确认：Web 无标准系统亮度控制时明确降级，禁止用 CSS 或遮罩虚假模拟系统亮度。
