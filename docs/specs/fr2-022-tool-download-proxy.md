...
> 状态：已交付@v0.23.0　·　关联 PRD：FR2-022　·　阶段：P2 `0.23.x`　·　分支：codex/fr2-022-tool-download-proxy
...
- [x] 定义工具源 registry 与平台/架构匹配。
- [x] 实现下载任务：代理、进度、sha256 校验、解压安全、版本探测。
- [x] 实现工具目录与路径热应用。
- [x] 新增工具源、下载 API，并扩展现有代理测试 API 支持下载源目标。
- [x] 前端设置/系统页增加工具下载与进度展示。
- [x] 接入审计事件。
- [x] 补单元测试：URL 校验、sha256、路径穿越、symlink/hardlink 拒绝、代理脱敏。
- [x] 补集成测试：httptest 下载源、checksum 错误拒绝、安装后 detect。
- [x] 补 E2E：选择工具源、下载入队、进度、失败重试。
