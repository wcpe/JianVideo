...
- [x] 在 `apps/wiki` 接入 PixiJS 样例页与 HLS 预览卡，独立可预览。
...
- **实测验收项**：性能是否达标以**本机 Benchmark 实测报告**为准，单元/e2e 全绿不替代它；本轮报告记录了帧耗时、长任务、纹理/内存与后端查询分位数，并按 [fr2-003] 门槛判定达标。
...
- PixiJS 已按本规格引入到 `packages/render-pixi`，后续升级仍需按依赖管理要求单独评估版本与兼容性。
