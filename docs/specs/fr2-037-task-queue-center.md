...
> 状态：已审核接受　·　关联 PRD：FR2-037　·　阶段：P2 `0.23.x`　·　分支：待定
...
- [ ] 新增统一 `tasks` model、repository 与状态机单元测试。
- [ ] 实现入队、幂等键、领取、进度更新、成功/失败/取消、重试退避。
- [ ] 实现 worker pool registry、按类型并发上限与公平调度策略。
- [ ] 实现启动恢复 `running` 任务。
- [ ] 新增任务 API 与 Space scoped 过滤。
- [ ] 兼容旧扫描/转码任务状态，保留旧 API 适配层。
- [ ] 接入任务审计事件接口。
- [ ] 补单元测试：状态机、优先级、公平调度、幂等、重试、取消、恢复。
- [ ] 补集成测试：SQLite 并发入队/领取、重启恢复、旧状态映射。
- [ ] 补 E2E：任务中心列表、过滤、取消、重试。
- [ ] 补 Benchmark：大任务表下列表/统计/领取延迟。
