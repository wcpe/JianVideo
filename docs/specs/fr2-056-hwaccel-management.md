...
> 状态：已交付@v0.23.0　·　关联 PRD：FR2-056　·　阶段：P2 `0.23.x`　·　分支：codex/fr2-056-hwaccel-management
...
- [x] 增加硬件转码设置 registry。
- [x] 转码执行器消费硬件偏好与 fallback 策略。
- [x] 系统页增加管理控件与状态说明。
- [x] 重测动作写审计事件。
- [x] 补单元测试：编码器选择、fallback、不可用策略。
- [x] 补集成测试：无硬件环境软件 fallback、缓存能力读取。
- [x] 补 E2E：选择策略、保存、刷新回读、强制重测。
