...
> 状态：已交付@v0.23.0　·　关联 PRD：FR2-052　·　阶段：P2 `0.23.x`　·　分支：`codex/fr2-052-library-kinds`
...
- 可选 `library_profile_json`：保存后续分型配置；本期只写空 JSON 预留。
...
- [x] 增加 `library_kind` schema 与默认值迁移。
- [x] 扩展库路径 API 与前端类型定义。
- [x] 增加内置分型说明 API。
- [x] 媒体库管理 UI 增加分型选择。
- [x] 扫描服务向元数据/推断链路传递分型上下文。
- [x] 补单元测试：分型校验、默认 mixed、非法值拒绝。
- [x] 补集成测试：旧库迁移默认值、新增/编辑库分型持久化。
- [x] 补 E2E：创建电影/剧集/家庭录像库并刷新展示。
