...
`1.0.0` 之前都属于 `0.y.z` 预稳定阶段。三段版本号不是从 `1.0.0` 后才开始，而是从当前开始就使用；区别是 `1.0.0` 后严格按 SemVer 语义执行：

- `1.0.1`：兼容修复，不改变功能契约。
- `1.1.0`：新增兼容功能。
- `2.0.0`：破坏性变更。
- 预发布：`1.0.0-rc.1`、`1.1.0-beta.1`。
...
| P8 RC | `0.29.x` | 1.0 候选 | 主要功能线冻结 | 只接受阻断修复、兼容修复、数据安全修复、性能回归修复、文档与验收补强；准备 `1.0.0-rc.1` |
| GA | `1.0.0` | 首个稳定版 | RC 验收通过 | 正式进入严格 SemVer 规则 |
...
- 准备 `1.0.0-rc.1`。
- RC 期间发现的阻断问题继续在 `0.29.x` patch 内修复，直到满足 GA 条件。
