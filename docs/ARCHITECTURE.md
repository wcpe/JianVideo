...
- **外部工具下载（FR2-022）**：`internal/tools` 封装工具源 registry、下载请求解析、受控安装器和任务 worker，支持 ffmpeg/ffprobe/ImageMagick `magick`。下载走 FR2-037 的系统级 `tool.download` 任务，payload 保存源元数据，worker 通过 `netproxy.ProxyFunc` 下载、按 SHA-256 校验、解压 zip/tar.gz、拒绝路径穿越/链接/非普通文件、探测 `-version` 后原子安装到数据目录 `tools/<tool>/<version>/`，再写入 `ffmpeg_path`/`ffprobe_path`/`magick_path` 并热应用。自定义 URL 默认要求 HTTPS；HTTP 仅允许本机测试源并需显式允许。`POST /api/system/proxy/test` 可带 `target` 做下载源连通性预检，但不会改写运行期代理。
