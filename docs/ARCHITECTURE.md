...
| `ai` | AI 可替换管线（FR2-011 / ADR-0059）与搜索/审核（FR2-012）：模型/节点注册表、结果表、嵌入式 `ai_embeddings` 向量、默认关闭门 `ai_enabled`、`ai.infer` worker、stub OCR/embedding、语义搜索余弦 topK、去重候选、结果 confirm/reject（manual 保护）、Space 隔离与 rebuild 保护；业务不依赖具体推理 SDK 与外部向量库 | → `db`, `tasks`, `settings`, `audit` |
