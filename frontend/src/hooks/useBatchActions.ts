...
async function runEach(ids: number[], fn: (id: number) => Promise<unknown>): Promise<{ ok: number; fail: number }> {
  let ok = 0
  let fail = 0
  for (const id of ids) {
    try {
...
export function useBatchActions(): BatchActions {
  ...
  const confirmAlbum = useCallback(async (albumID: number) => {
  ...
  const confirmTag = useCallback(async (tag: { tag_id?: number; name?: string }) => {
    setTagOpened(false)
