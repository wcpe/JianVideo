...
describe('useBatchActions（FR-91）', () => {
  it('加入相册：逐项调 addAlbumItem，toast 显示成功计数', async () => {
    ...
    await act(async () => { await result.current.openAddToAlbum([1, 2, 3]) })
    await waitFor(() => expect(result.current.modalState.albums).toHaveLength(1))

    await act(async () => { await result.current.modalState.confirmAlbum(7) })
    expect(addAlbumItemMock).toHaveBeenCalledTimes(3)
    expect(addAlbumItemMock).toHaveBeenCalledWith(7, 1)
    expect(showMock).toHaveBeenCalledWith(expect.objectContaining({ color: 'green', message: '已加入相册 3 项' }))
  })
  ...
  it('打标签：单项失败计入失败计数，toast 黄色', async () => {
    ...
    await act(async () => { await result.current.openAddTag([1, 2]) })
    await act(async () => { await result.current.modalState.confirmTag({ tag_id: 5 }) })

    expect(addMediaTagMock).toHaveBeenCalledTimes(2)
    expect(showMock).toHaveBeenCalledWith(expect.objectContaining({ color: 'yellow', message: '已打标签 1 项，失败 1 项' }))
  })
  ...
  it('打包下载：fetch 成功时触发 blob 下载', async () => {
    ...
    const { result } = renderHook(() => useBatchActions())
    await act(async () => { await result.current.download([1, 2]) })
  ...
  it('打包下载：后端返回错误时弹红色 toast、不触发下载', async () => {
    ...
    expect(showMock).toHaveBeenCalledWith(expect.objectContaining({ color: 'red', message: '选中文件总大小超过上限' }))
    vi.unstubAllGlobals()
  })
})
