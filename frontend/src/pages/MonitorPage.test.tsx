...
const sampleMetrics: SystemMetrics = {
  ...
  current: { t: '2026-06-27T12:00:00Z', cpu_percent: 47.2, mem_used_bytes: 202000000, mem_sys_bytes: 540000000, disk_used_bytes: 1980900000000, disk_total_bytes: 2600000000000, transcode_active: 3, goroutines: 122 },
}
...
describe('MonitorPage', () => {
  ...
  it('提供 range 选择（默认 24h），切到 1h 重拉并仍渲染当前值卡', async () => {
    ...
    server.use(http.get('*/api/system/metrics', ({ request }) => {
  ...
  it('请求失败时静默降级：不抛错、显占位（保留加载兜底而非崩溃）', async () => {
    server.use(http.get('*/api/system/metrics', () => HttpResponse.json({ message: '炸了' }, { status: 500 })))
    renderPage()
