...
describe('AlbumsPage', () => {
  ...
  it('点击相册查看其内容（成员列表）', async () => {
    server.use(
      ...
      http.get('*/api/albums/5/items', () =>
        HttpResponse.json({
          items: [
            {
              id: 1,
              library_id: 1,
              file_path: 'D:\\A\\星际穿越.mkv',
              file_name: '星际穿越.mkv',
              file_size: 100,
              format: 'mkv',
              video_codec: 'h264',
  ...
  it('无相册时渲染统一空态（FR-98）', async () => {
    server.use(http.get('*/api/albums', () => HttpResponse.json({ items: [] })));
    renderPage();
  ...
  it('加载相册失败时渲染插画+重试空态（FR-98）', async () => {
    server.use(http.get('*/api/albums', () => new HttpResponse(null, { status: 500 })));
    renderPage();
  ...
  it('首屏加载相册列表时渲染骨架屏（FR-98）', async () => {
    ...
    server.use(http.get('*/api/albums', () => new Promise(() => {})));
