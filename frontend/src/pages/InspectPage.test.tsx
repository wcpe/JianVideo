...
describe('InspectPage', () => {
  ...
  it('按问题类型分组列出问题媒体', async () => {
    server.use(
      http.get('*/api/library/health/status', () => HttpResponse.json(completedStatus)),
      http.get('*/api/library/health/issues', () =>
        HttpResponse.json({
          items: [
  ...
  it('巡检进行中展示进度', async () => {
    server.use(
      http.get('*/api/library/health/status', () =>
        HttpResponse.json({ ...idleStatus, status: 'scanning', total: 10, checked: 4 }),
      ),
      http.get('*/api/library/health/issues', () => HttpResponse.json({ items: [] })),
    );
    renderPage();
