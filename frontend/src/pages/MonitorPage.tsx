...
import { Stack, Title, Text, Card, SegmentedControl, SimpleGrid, Loader, Center, Group } from '@mantine/core'
...
export default function MonitorPage() {
  ...
  useEffect(() => {
    ...
    const fetchOnce = () => {
      getSystemMetrics(range)
        ...
        .catch(() => { /* 轮询失败本就静默重试，不弹 toast；保留上次数据不强制清空 */ })
  ...
  return (
    <Stack gap="lg">
      ...
      <Text size="sm" c="dimmed">CPU / 内存 / 磁盘 / 转码并发的当前值与随时间的变动（每 15 秒自动刷新）。</Text>
  ...
  return (
    <Stack gap="lg">
      ...
      <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing="md">
        <MetricCard label="CPU 使用率" value={`${current.cpu_percent.toFixed(1)}%`} sparkline={cpuSpark} />
        <MetricCard label="内存（进程已用）" value={formatBytes(current.mem_used_bytes)} sparkline={memSpark} />
        <MetricCard
          label={`磁盘（已用 ${diskPct}%）`}
          value={`${formatBytes(current.disk_used_bytes)} / ${formatBytes(current.disk_total_bytes)}`}
          sparkline={diskSpark}
        />
        <MetricCard label="转码并发" value={current.transcode_active.toLocaleString()} sparkline={transcodeSpark} />
      </SimpleGrid>

      {/* CPU 使用率时序折线（x=points[].t，可 hover 看每点精确值） */}
      <Card withBorder padding="md" radius="md">
        <Text fw={600} mb="xs">CPU 使用率趋势</Text>
        <Text size="xs" c="dimmed" mb="sm">系统 CPU 使用率随时间的变动（%）。</Text>
        <TrendChart
      ...
      {/* 内存时序折线（进程已用，字节→人类可读） */}
      <Card withBorder padding="md" radius="md">
        <Text fw={600} mb="xs">内存趋势</Text>
        <Text size="xs" c="dimmed" mb="sm">进程已用内存随时间的变动。</Text>
        <TrendChart
          data={points as unknown as Array<Record<string, unknown>>}
          xKey="t"
      ...
      {/* 转码并发时序折线（活跃会话数） */}
      <Card withBorder padding="md" radius="md">
        <Text fw={600} mb="xs">转码并发趋势</Text>
        <Text size="xs" c="dimmed" mb="sm">活跃转码 / 播放会话数随时间的变动。</Text>
        <TrendChart
          data={points as unknown as Array<Record<string, unknown>>}
          xKey="t"
