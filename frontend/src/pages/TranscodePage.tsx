...
export default function TranscodePage() {
  ...
  const loadPresets = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({
        title: '加载失败',
        message: err instanceof Error ? err.message : '无法加载预设',
        color: 'red',
        autoClose: 3000,
      });
    } finally {
  ...
  const openEdit = (p: TranscodePreset) =>
    setEdit({
      opened: true,
      editing: p,
      name: p.name,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({
        title: '保存失败',
        message: err instanceof Error ? err.message : '保存预设失败',
        color: 'red',
        autoClose: 3000,
      });
  ...
  return (
    <Stack gap="md">
              ...
              <Card key={t.id} withBorder padding="sm" radius="md">
                <Group justify="space-between" wrap="nowrap">
                  <Text size="sm" truncate>
                    媒体 #{t.media_id} · {t.codec.toUpperCase()}
                  </Text>
                ...
                {t.status === 'error' && t.error && (
                  <Text size="xs" c="red" mt={4} truncate>
                    {t.error}
                  </Text>
                )}
              </Card>
      ...
      <Modal
        opened={edit.opened}
        onClose={() => setEdit(initEdit)}
        title={edit.editing ? '编辑预设' : '新建预设'}
        centered
        size="sm"
      >
        <Stack gap="md">
