...
describe('SystemPage（FR-113 区块化渲染）', () => {
  beforeEach(() => {
    ...
    server.use(
      http.get('*/api/settings', () =>
        HttpResponse.json({ settings: { update_channel: 'stable' } }),
      ),
    );
  });
  ...
  it('点「回滚到上一版」弹模态框，确认触发回滚、取消不触发（FR-62）', async () => {
    ...
    await user.click(
      await screen.findByRole('button', { name: /回滚到上一版/ }, { timeout: 3000 }),
    );
        ...
        HttpResponse.json(
          {
            code: 'UPDATE_FAILED',
            message:
              '更新失败：下载失败: Get "https://github.com/wcpe/JianVideo/releases/download/v0.17.0/jianvideo-windows-amd64.exe": read tcp 192.168.31.123:2946->140.82.112.4:443: wsarecv: A connection attempt failed because the connected party did not properly respond',
          },
          { status: 500 },
        ),
      ),
  ...
  it('更新失败后展示「重试」按钮，点击重新触发更新（FR-90）', async () => {
    ...
    server.use(
      http.post('*/api/system/update/apply', () => {
        ...
        if (applyCalls === 1) {
          return HttpResponse.json(
            { code: 'UPDATE_FAILED', message: '更新失败：下载中断' },
            { status: 500 },
          );
        }
        return HttpResponse.json({ status: 'updating', message: '更新已应用，服务即将重启' });
