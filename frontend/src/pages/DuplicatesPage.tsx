...
export default function DuplicatesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({ title: '加载失败', message: err instanceof Error ? err.message : '无法加载重复项', color: 'red', autoClose: 3000 })
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({ title: '扫描完成', message: `本次计算 ${computed} 项媒体哈希`, color: 'green', autoClose: 3000 })
      await load()
    } catch (err) {
      notifications.show({ title: '扫描失败', message: err instanceof Error ? err.message : '去重扫描失败', color: 'red', autoClose: 3000 })
    } finally {
      setScanning(false)
    }
  }, [load])
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({ title: '已移入回收站', message: `已删除 ${deleted} 项（可在回收站还原）`, color: 'green', autoClose: 3000 })
      setSelected(new Set())
      await load()
    } catch (err) {
  ...
  return (
    <Stack gap="md">
      <Group justify="space-between" align="flex-end">
        ...
        <Group gap="sm">
          <Button variant="light" color="purple" leftSection={<IconScan size={16} />} loading={scanning} onClick={handleScan}>
            扫描重复项
          </Button>
          <Button color="red" leftSection={<IconTrash size={16} />} loading={deleting} disabled={selected.size === 0} onClick={handleDelete}>
