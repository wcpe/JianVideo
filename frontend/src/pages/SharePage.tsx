...
import { Container, Title, Text, Loader, Center, SimpleGrid, Card, Image, Button, Group, Stack, Modal, PasswordInput } from '@mantine/core'
...
function PasswordGate({ onSubmit, submitting, wrong }: { onSubmit: (pwd: string) => void; submitting: boolean; wrong: boolean }) {
  const [pwd, setPwd] = useState('')
  return (
    <Center h="100vh">
      <Stack w={320} component="form" onSubmit={e => { e.preventDefault(); onSubmit(pwd) }}>
        <Group gap="xs" justify="center"><IconLock size={20} /><Title order={4}>该分享需要密码</Title></Group>
        <PasswordInput value={pwd} onChange={e => setPwd(e.currentTarget.value)} placeholder="请输入访问密码" aria-label="访问密码" autoFocus
          error={wrong ? '分享不存在或已过期' : undefined} />
        <Button type="submit" loading={submitting}>访问</Button>
      </Stack>
    </Center>
  )
}
...
export default function SharePage() {
  ...
  if (error) return <Center h="100vh"><Text c="dimmed">{error}</Text></Center>
  if (needPassword) return <PasswordGate onSubmit={submitPassword} submitting={submitting} wrong={wrongPassword} />
  if (!info) return <Center h="100vh"><Text c="dimmed">分享不存在或已过期</Text></Center>
