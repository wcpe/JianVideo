...
import { Stack, Title, Text, Loader, Center, Box } from '@mantine/core'
...
async function fetchAllGeotagged(): Promise<MediaFile[]> {
  ...
  let page = 1
  for (;;) {
    const res = await libApi.getMediaFiles({ has_gps: true, page, page_size: 100 })
    all.push(...res.items)
...
/** 照片地图视图（FR-39）：基于 EXIF GPS 在 OSM 在线瓦片上展示照片地理分布 */
