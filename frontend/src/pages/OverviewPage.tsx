...
import {
  IconPhoto, IconVideo, IconClock, IconDatabase, IconLibrary, IconAlbum,
 ...
 */
function KpiCard({ icon, value, title, subtitle }: {
    ...
    >
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle
          ...
          fill="none" stroke="var(--mantine-color-default-border)" strokeWidth={stroke}
        />
        {/* 进度环：品牌紫，从 12 点方向起绕 */}
        <circle
          cx={size / 2} cy={size / 2} r={radius}
          fill="none" stroke="var(--mantine-color-purple-6)" strokeWidth={stroke}
      ...
      >
        <Text size="lg" fw={700}>{clamped}%</Text>
      </Box>
    </Box>
  )
}
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    ...
    listAlbums().then((items) => { if (active) setAlbumCount(items.length) }).catch(() => { if (active) setAlbumCount(null) })
    getWatchStats().then((s) => { if (active) setWatch(s) }).catch(() => { if (active) setWatch(null) })
    getSystemInfo().then((info) => { if (active) setSystem(info) }).catch(() => { if (active) setSystem(null) })
    getHealthStatus().then((st) => { if (active) setIssueCount(st.issue_count) }).catch(() => { if (active) setIssueCount(null) })
    getScanTasks().then((res) => { if (active) setScanCurrent(res.current) }).catch(() => { if (active) setScanCurrent(null) })
    listTranscodeTasks()
      .then((tasks) => {
        if (!active) return
      ...
      .catch(() => { if (active) setTranscode(null) })
  ...
  return (
    <Stack gap="lg">
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
        <Card withBorder padding="md" radius="md">
          <Text fw={600} mb="sm">媒体构成</Text>
          <Stack gap="md">
            <div>
              <Group justify="space-between" mb={4}>
                <Group gap={6}><IconVideo size={16} /><Text size="sm">视频</Text></Group>
                <Text size="sm" c="dimmed">{videoCount.toLocaleString()}（{videoPercent}%）</Text>
              </Group>
              <Progress value={videoPercent} color="purple" size="lg" aria-label={`视频占比 ${videoPercent}%`} />
            </div>
            <div>
              <Group justify="space-between" mb={4}>
                <Group gap={6}><IconPhoto size={16} /><Text size="sm">图片</Text></Group>
                <Text size="sm" c="dimmed">{imageCount.toLocaleString()}（{imagePercent}%）</Text>
              </Group>
              <Progress value={imagePercent} color="grape" size="lg" aria-label={`图片占比 ${imagePercent}%`} />
            </div>
          </Stack>
        </Card>

        <Card withBorder padding="md" radius="md">
          <Text fw={600} mb="sm">各库分布</Text>
          {libraries.length === 0 ? (
            <Text size="sm" c="dimmed">暂无媒体库数据。</Text>
          ) : (
            <Stack gap="xs">
              {libraries.map((l) => (
        ...
        <Card withBorder padding="md" radius="md">
          <Group justify="space-between" mb="sm">
            ...
            <Link to="/stats" style={{ textDecoration: 'none' }}>
              <Text size="sm" c="purple">查看全部 ›</Text>
            </Link>
          </Group>
          <Group gap="lg" align="center" wrap="nowrap">
            ...
            <Stack gap="xs">
              <Group gap={6}>
                <Badge color="purple" variant="filled" size="sm">已看</Badge>
                <Text size="sm">{watched.toLocaleString()}</Text>
              </Group>
              <Group gap={6}>
                <Badge color="gray" variant="light" size="sm" styles={{ label: { color: 'var(--mantine-color-gray-7)' } }}>未看</Badge>
                <Text size="sm">{unwatched.toLocaleString()}</Text>
              </Group>
              <Text size="xs" c="dimmed">共 {watchTotal.toLocaleString()} 个媒体</Text>
            </Stack>
          </Group>
        </Card>

        <Card withBorder padding="md" radius="md">
          <Text fw={600} mb="sm">系统状态</Text>
          {system ? (
            <Stack gap="xs">
              <Group justify="space-between">
                <Text size="sm" c="dimmed">FFmpeg</Text>
                <Badge color={system.ffmpeg.available ? 'green' : 'red'} size="sm">
                  {system.ffmpeg.available ? '可用' : '不可用'}
                </Badge>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="dimmed">硬件加速</Text>
                <Text size="sm">{system.hwaccel.preferred || '软件编码'}</Text>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="dimmed">运行时长</Text>
                <Text size="sm">{system.runtime ? formatUptime(system.runtime.uptime_seconds) : '—'}</Text>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="dimmed">运行内存</Text>
                <Text size="sm">{system.runtime ? formatBytes(system.runtime.mem_alloc) : '—'}</Text>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="dimmed">巡检问题</Text>
                {issueCount === null ? (
                  <Text size="sm" c="dimmed">—</Text>
                ) : (
                  <Badge color={issueCount > 0 ? 'orange' : 'green'} size="sm">{issueCount} 个</Badge>
                )}
              </Group>
            </Stack>
          ) : (
            <Text size="sm" c="dimmed">系统信息加载失败。</Text>
          )}
        </Card>
      </SimpleGrid>
      ...
      <Card withBorder padding="md" radius="md">
        <Text fw={600} mb="sm">任务队列</Text>
        <Stack gap="md">
          <div>
            <Text size="sm" c="dimmed" mb={4}>扫描任务</Text>
            {scanCurrent ? (
              <div>
                <Group justify="space-between" mb={2}>
                  <Text size="sm">库 #{scanCurrent.library_id} 扫描中</Text>
                  <Text size="sm" c="dimmed">{scanCurrent.scanned_files} / {scanCurrent.total_files}</Text>
                </Group>
                <Progress
                  value={scanCurrent.total_files > 0 ? (scanCurrent.scanned_files / scanCurrent.total_files) * 100 : 0}
                  color="purple" size="sm" animated
          ...
          <div>
            <Text size="sm" c="dimmed" mb={4}>转码任务</Text>
            <Group gap="lg">
              <Group gap={6}><Badge color="gray" variant="light" size="sm">待处理</Badge><Text size="sm">{transcode?.pending ?? 0}</Text></Group>
              <Group gap={6}><Badge color="blue" variant="light" size="sm">进行中</Badge><Text size="sm">{transcode?.running ?? 0}</Text></Group>
              <Group gap={6}><Badge color="green" variant="light" size="sm">已完成</Badge><Text size="sm">{transcode?.completed ?? 0}</Text></Group>
            </Group>
          </div>
        </Stack>
      </Card>
