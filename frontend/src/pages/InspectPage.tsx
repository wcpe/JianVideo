...
export default function InspectPage() {
  ...
  const loadIssues = useCallback(async () => {
    ...
    try {
      ...
      notifications.show({
        title: '加载失败',
        message: err instanceof Error ? err.message : '无法加载问题清单',
        color: 'red',
        autoClose: 3000,
      });
    } finally {
  ...
  const handleScan = useCallback(async () => {
    try {
      ...
      notifications.show({
        title: '触发失败',
        message: err instanceof Error ? err.message : '无法触发巡检',
        color: 'red',
        autoClose: 3000,
      });
    }
  ...
  const grouped = GROUP_ORDER.map((type) => ({
    type,
    items: issues.filter((i) => i.issue_type === type),
  })).filter((g) => g.items.length > 0);

  const percent =
    status && status.total > 0 ? Math.round((status.checked / status.total) * 100) : 0;
        ...
        <Stack gap="lg">
          {grouped.map((group) => (
            <Stack key={group.type} gap="xs">
              <Group gap="xs">
                <Badge color={ISSUE_META[group.type].color} variant="light">
                  {ISSUE_META[group.type].label}
                </Badge>
                <Text size="sm" c="dimmed">
