...
function buildReport(info: SystemInfo | null, codec: CodecTestResult | null): string {
  ...
  if (info) {
    ...
    if (info.runtime) {
      ...
      lines.push(`数据库路径: ${rt.db_path}`);
      lines.push(
        `运行内存: ${formatBytes(rt.mem_alloc)} / ${formatBytes(rt.mem_sys)}（已用/申请）`,
      );
      lines.push(`GC 次数: ${rt.num_gc}`);
      lines.push(`GOMAXPROCS: ${rt.gomaxprocs}`);
    }
    ...
    lines.push(
      `实测来源: ${info.hwaccel.tested_at ? `${info.hwaccel.from_cache ? '缓存' : '实测'} @ ${info.hwaccel.tested_at}` : '未测'}`,
    );
    for (const cap of info.hwaccel.available) {
      for (const c of cap.codecs) {
        lines.push(
          `硬件加速: ${cap.family}/${c.codec} ${c.encoder} 试编码:${c.tested_ok ? '成功' : '失败'}`,
        );
      }
    }
  }
  if (codec) {
    ...
    if (!codec.ffmpeg_available) {
      ...
      for (const r of codec.results) {
        ...
        lines.push(
          `${r.encoder} [${r.family}/${r.codec}] 编入:${r.compiled ? '是' : '否'} 试编码:${status}${r.detail ? ` 详情:${r.detail}` : ''}`,
        );
      }
    }
  }
...
const UPDATE_NETWORK_ERR_RE =
  /下载失败|超时|timeout|wsarecv|wsasend|dial|connection|connect|refused|reset|EOF|no such host|did not properly respond|网络不可达|网络异常|i\/o timeout/i;
...
export default function SystemPage({ section }: { section: SystemSection }) {
  ...
  useEffect(() => {
    ...
    systemApi
      ...
      .catch((err) => {
        if (active) setInfoError(extractErrorMessage(err, '加载系统信息失败'));
      })
      .finally(() => {
  ...
  const handleChannelChange = useCallback((v: string) => {
    ...
    setChannel(ch);
    updateSettings({ [SETTING_KEY_UPDATE_CHANNEL]: ch }).catch(() => {
      /* 持久化失败不阻塞使用 */
    });
  }, []);
                  ...
                  <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="xs">
                    ...
                    {info.runtime && (
                      <>
                        ...
                        <InfoRow
                          label="运行时长"
                          value={formatUptime(info.runtime.uptime_seconds)}
                        />
                        <InfoRow
                          label="运行内存（已用/申请）"
                          value={`${formatBytes(info.runtime.mem_alloc)} / ${formatBytes(info.runtime.mem_sys)}`}
                        />
              ...
              <Group gap="xs" align="center">
                ...
                {info.hwaccel.codecs.length === 0 ? (
                  ...
                  info.hwaccel.codecs.map((c) => (
                    <Badge key={c} variant="light" color="green">
                      {c}
                    </Badge>
                  ))
                )}
      ...
      {section === 'update' && (
        <>
          <Card id="update" withBorder padding="md" radius="md">
            ...
            {updateBusy &&
              updateProgress &&
              (updateProgress.state === 'downloading' || updateProgress.state === 'verifying') && (
                <Box mb="sm">
                  <Group justify="space-between" mb={4}>
                    <Text size="sm" c="dimmed">
            ...
            {updateInfo ? (
              ...
              !updateError && (
                <Text size="sm" c="dimmed">
                  暂无本地缓存的检查结果。点击「检查更新」从 GitHub Releases 强制重查最新版本。
                </Text>
              )
            )}
          </Card>
          ...
          <Modal opened={applyModalOpened} onClose={applyModal.close} title="确认更新" centered>
            <Stack gap="md">
              ...
              <Group justify="flex-end" gap="xs">
                <Button variant="default" onClick={applyModal.close}>
                  取消
                </Button>
                <Button
                  color="purple"
          ...
          >
            <Stack gap="md">
              ...
              <Group justify="flex-end" gap="xs">
                <Button variant="default" onClick={rollbackModal.close}>
                  取消
                </Button>
                <Button
                  color="red"
