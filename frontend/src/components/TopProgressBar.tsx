...
export default function TopProgressBar() {
  ...
  useEffect(() => {
    ...
    setProgress(10);
    // 爬升至 ~80%
    timers.current.push(setTimeout(() => setProgress(CLIMB_TO), CLIMB_DELAY));
    // 补满
    timers.current.push(setTimeout(() => setProgress(100), FINISH_DELAY));
    // 补满后淡出并复位
    timers.current.push(
      setTimeout(() => {
        setVisible(false);
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: reducedMotion.current ? 'none' : 'width 180ms ease, opacity 200ms ease',
