...
describe('VideoPlayer 续播与观看状态（FR-44）', () => {
  ...
  it('播放中按节流上报位置', () => {
    ...
    act(() => { video.currentTime = 12; video.dispatchEvent(new Event('timeupdate')) })
    expect(onPositionReport).toHaveBeenCalledWith(12)

    // 紧接着 +5s 未达节流间隔，不重复上报
    act(() => { video.currentTime = 17; video.dispatchEvent(new Event('timeupdate')) })
    expect(onPositionReport).toHaveBeenCalledTimes(1)

    // 再 +10s 达到间隔，再次上报
    act(() => { video.currentTime = 23; video.dispatchEvent(new Event('timeupdate')) })
  ...
  it('暂停时补报一次当前位置', () => {
    ...
    act(() => { video.currentTime = 5; video.dispatchEvent(new Event('pause')) })
  ...
  it('接近片尾触发 onEnded 一次', () => {
    ...
    // 剩余 < 15s 视为看完
    act(() => { video.currentTime = 6590; video.dispatchEvent(new Event('timeupdate')) })
    expect(onEnded).toHaveBeenCalledTimes(1)

    // 再次 timeupdate 不重复触发
    act(() => { video.currentTime = 6595; video.dispatchEvent(new Event('timeupdate')) })
    expect(onEnded).toHaveBeenCalledTimes(1)
  })
})
