 ...
 * 标签列表自取自管，新建后刷新下拉。
 */
export default function MediaFilterBar({ favorite, onFavoriteChange, tagId, onTagIdChange }: MediaFilterBarProps) {
  ...
  return (
    <Group gap="sm">
      ...
      <NativeSelect
        ...
        data={[{ value: '0', label: '全部标签' }, ...tags.map((t) => ({ value: String(t.id), label: t.name }))]}
        value={String(tagId)}
      ...
      {newTagOpened && (
        <Group gap="xs">
          <TextInput
            ...
            size="sm"
          />
          <Button size="xs" loading={creating} onClick={handleCreateTag}>创建</Button>
        </Group>
      )}
    </Group>
  )
