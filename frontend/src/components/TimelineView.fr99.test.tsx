...
const video = (id: number, over: Partial<MediaFile> = {}): MediaFile => ({
  id, library_id: 1, file_path: `/x/v${id}.mp4`, file_name: `v${id}.mp4`,
  file_size: 1024 * 1024, format: 'mp4', video_codec: 'h264', audio_codec: 'aac', duration: 95,
  width: 1920, height: 1080, bitrate: 0, subtitle_tracks: '',
  added_at: '2025-01-09T00:00:00Z', modified_at: '2025-01-09T00:00:00Z', ...over,
})
const image = (id: number): MediaFile => video(id, { format: 'png', file_name: `p${id}.png`, duration: 0 })
...
describe('TimelineView 媒体卡与网格重设计（FR-99）', () => {
  ...
  it('选中 ≥1 项时浮现 sticky 批量条，含批量按钮且回调可触发', () => {
    ...
    renderView({ onBatchDelete, onBatchAddToAlbum, onBatchAddTag: vi.fn(), onBatchDownload: vi.fn() })
    // 未选中：无批量条
