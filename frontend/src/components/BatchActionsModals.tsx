...
export default function BatchActionsModals({ state }: BatchActionsModalsProps) {
  const {
    albumOpened, albums, loadingAlbums, confirmAlbum, closeAlbum,
    tagOpened, tags, loadingTags, confirmTag, closeTag,
  } = state
  ...
  useEffect(() => {
    if (tagOpened) { setTagID(tags[0] ? String(tags[0].id) : ''); setNewTagName('') }
  }, [tagOpened, tags])
    ...
    <>
      ...
      <Modal opened={albumOpened} onClose={closeAlbum} title="加入相册" centered>
        {loadingAlbums ? (
          ...
          <Text c="dimmed" size="sm">暂无相册，请先到相册页创建</Text>
        ) : (
          <Stack gap="md">
            <NativeSelect
              label="选择相册"
            ...
            <Group justify="flex-end">
              <Button variant="default" onClick={closeAlbum}>取消</Button>
              <Button disabled={!albumID} onClick={() => confirmAlbum(Number(albumID))}>加入</Button>
            </Group>
          </Stack>
        )}
      </Modal>
          ...
          <Stack gap="md">
            {tags.length > 0 && (
              <NativeSelect
                ...
                aria-label="选择已有标签"
                value={tagID}
                onChange={(e) => { setTagID(e.currentTarget.value); setNewTagName('') }}
                data={[{ value: '', label: '（不选）' }, ...tags.map((t) => ({ value: String(t.id), label: t.name }))]}
              />
            )}
            <TextInput
              label="或新建标签"
            ...
            />
            <Group justify="flex-end">
              <Button variant="default" onClick={closeTag}>取消</Button>
              <Button disabled={!newTagName.trim() && !tagID} onClick={handleConfirmTag}>打标签</Button>
            </Group>
          </Stack>
        )}
      </Modal>
