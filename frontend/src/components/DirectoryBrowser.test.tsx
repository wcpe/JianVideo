...
function renderBrowser(opts: {
  ...
  sort?: DirSort;
  onOpenFile?: (f: MediaFile, i: number) => void;
  ...
  return render(
    <MantineProvider>
      <DirectoryBrowser
        ...
        onEnterDir={opts.onEnterDir ?? (() => {})}
        onBreadcrumbNavigate={() => {}}
        onErrorClose={() => {}}
        onOpenFile={opts.onOpenFile ?? (() => {})}
        displayMode={opts.displayMode ?? 'list'}
        sort={opts.sort ?? 'name'}
      />
    </MantineProvider>,
  );
...
describe('sortFiles（FR-33）', () => {
  it('按大小/类型/名称排序', () => {
    const files = [
      file({ id: 1, file_name: 'b.png', format: 'png', file_size: 300 }),
  ...
  it('按修改时间排序', () => {
    const files = [
      file({ id: 1, modified_at: '2025-03-01T00:00:00Z' }),
