...
import {
  IconTrash, IconChecks, IconArrowsExchange, IconCheckbox, IconSquare,
  IconPhotoPlus, IconTag, IconDownload,
...
export default function MediaContextMenu({
  ...
  onAddToAlbum, onAddTag, onDownload,
          ...
          <>
            ...
            {onAddToAlbum && (
              <Menu.Item
                ...
                onClick={() => { if (state) onAddToAlbum(state.targetId) }}
              >
                {`加入相册${suffix}`}
              </Menu.Item>
            )}
            {onAddTag && (
              <Menu.Item
                ...
                onClick={() => { if (state) onAddTag(state.targetId) }}
              >
                {`打标签${suffix}`}
              </Menu.Item>
            )}
            {onDownload && (
              <Menu.Item
                ...
                onClick={() => { if (state) onDownload(state.targetId) }}
              >
                {`打包下载${suffix}`}
              </Menu.Item>
            )}
