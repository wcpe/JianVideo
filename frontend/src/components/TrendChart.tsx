 ...
 */
export default function TrendChart({ data, xKey, series, height = 240, valueFormatter }: TrendChartProps) {
  ...
  if (!data || data.length === 0) {
    return (
      ...
      >
        <span style={{ color: 'var(--mantine-color-dimmed)', fontSize: 'var(--mantine-font-size-sm)' }}>暂无数据</span>
      </div>
    )
  }
  ...
  return (
    ...
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey={xKey} tick={{ fontSize: 12, fill: AXIS_COLOR }} stroke={GRID_COLOR} minTickGap={24} />
