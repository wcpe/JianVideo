...
describe('AppLayout 侧栏激活态 pill（FR-95）', () => {
  ...
  it('当前路由 /browse：桌面 navbar 中「目录」项标记为激活态', () => {
    ...
    expect(
      active[0].querySelector('a[href="/browse"]') ??
        active[0].closest('a[href="/browse"]') ??
        navbar.querySelector('a[href="/browse"][data-active="true"]'),
    ).not.toBeNull();
  });
...
describe('AppLayout 导航交互完善（FR-115）', () => {
  ...
  it('「概览」导航项指向 / 首页，「时间轴」指向 /timeline（FR-117 迁址）', () => {
    ...
    expect(within(navbar).getByRole('link', { name: '时间轴' })).toHaveAttribute(
      'href',
      '/timeline',
    );
  });
