...
async function mockGetHealthStatus(): Promise<HealthScanStatus> {
  await mockDelay(80)
  return { status: 'completed', total: 0, checked: 0, issue_count: 0, error: '', started_at: '', completed_at: new Date().toISOString() }
}
...
export function startHealthScan() { return useMock ? mockStartHealthScan() : realStartHealthScan() }
export function getHealthStatus() { return useMock ? mockGetHealthStatus() : realGetHealthStatus() }
export function getHealthIssues() { return useMock ? mockGetHealthIssues() : realGetHealthIssues() }
