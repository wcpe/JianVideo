...
async function realGetSystemMetrics(range: string): Promise<SystemMetrics> {
  ...
  const res = await client.get<SystemMetrics>('/api/system/metrics', {
    params: { range },
    silent: true,
  });
  return res.data;
...
async function mockGetSystemMetrics(range: string): Promise<SystemMetrics> {
  ...
  return {
    ...
    points: [
      ...
      {
        t: '2026-06-27T13:00:00Z',
        cpu_percent: 55.7,
        mem_used_bytes: 210000000,
        mem_sys_bytes: 545000000,
        disk_used_bytes: 1980600000000,
        disk_total_bytes: 2600000000000,
        transcode_active: 3,
        goroutines: 130,
      },
