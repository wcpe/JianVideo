...
async function realEnqueueTask(mediaID: number, presetID: number): Promise<number> {
  try {
    const res = await client.post<{ status: string; task_id: number }>('/api/transcode/tasks', {
      media_id: mediaID,
      preset_id: presetID,
    });
...
export function listPresets() {
  return useMock ? mockListPresets() : realListPresets();
}
export function createPreset(input: PresetInput) {
  return useMock ? mockCreatePreset(input) : realCreatePreset(input);
}
export function updatePreset(id: number, input: PresetInput) {
  return useMock ? mockUpdatePreset(id, input) : realUpdatePreset(id, input);
}
export function deletePreset(id: number) {
  return useMock ? mockDeletePreset(id) : realDeletePreset(id);
}
export function enqueueTranscodeTask(mediaID: number, presetID: number) {
  return useMock ? mockEnqueueTask(mediaID, presetID) : realEnqueueTask(mediaID, presetID);
}
export function listTranscodeTasks(status?: string) {
  return useMock ? mockListTasks(status) : realListTasks(status);
}
