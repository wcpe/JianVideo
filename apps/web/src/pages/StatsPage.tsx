  ...
  useEffect(() => {
    ...
    getWatchStats()
      ...
      .catch((err) => {
        notifications.show({
          title: '加载失败',
          message: err instanceof Error ? err.message : '无法加载观看统计',
          color: 'red',
          autoClose: 3000,
        });
      })
    ...
    getContinueWatching()
      .then((items) => {
        if (active) setContinueCount(items.length);
      })
      .catch(() => {
        if (active) setContinueCount(null);
      });
    return () => {
      active = false;
    };
    ...
    <Stack gap="lg">
      ...
      <Tabs value={activeTab} onChange={handleTabChange} keepMounted={false}>
        <Tabs.List mb="md">
          ...
          <Tabs.Tab value={TAB_MEDIA} leftSection={<IconPhoto size={16} />}>
            媒体
          </Tabs.Tab>
        </Tabs.List>
  ...
  return (
    <Stack gap="lg">
      <Text size="sm" c="dimmed">
        基于观看状态（FR-44）汇总：看了多少、最近活跃度、续播停留位置、各库与各格式分布、最常看的视频。
      </Text>
      ...
      <Card withBorder padding="md" radius="md">
        <Group justify="space-between" mb="xs">
          ...
          <Text size="sm" c="dimmed">
            已看 {stats.watched} / 共 {stats.total}（{watchedPercent}%）
          </Text>
        </Group>
        <Progress.Root size="xl">
          <Progress.Section
            ...
            color="purple"
            aria-label={`观看进度 ${watchedPercent}%`}
          >
        ...
        <Group gap="lg" mt="xs">
          <Group gap={6}>
            ...
            <Text size="sm">{stats.watched}</Text>
          </Group>
          ...
          <Group gap={6}>
            ...
            <Text size="sm">{stats.unwatched}</Text>
          </Group>
        </Group>
      </Card>
      ...
      {!noActivity && (
        <Card withBorder padding="md" radius="md">
          ...
          <Text size="xs" c="dimmed" mb="sm">
            有续播进度的视频停留在播放时长哪个区间——越深表示该区间停留的视频越多。
          </Text>
          {heatmapMax === 0 ? (
            <Text size="sm" c="dimmed">
              暂无续播进度数据。
                ...
                <Stack key={i} gap={2} align="center">
                  <div
                    ...
                    style={{
                      ...
                      height: 40,
                      borderRadius: 6,
                      background: heatColor(value, heatmapMax),
                      display: 'flex',
                      alignItems: 'center',
                ...
                <Stack key={b.date} gap={2} align="center">
                  ...
                  >
                    ...
                    <div
                      style={{
                        width: '100%',
                        height: `${timelineMax > 0 ? Math.round((b.count / timelineMax) * 100) : 0}%`,
            ...
            <Stack gap="xs">
              {stats.by_library.map((l) => (
                <div key={l.library_id}>
                  ...
                  <Progress
                    value={libraryMax > 0 ? (l.watched / libraryMax) * 100 : 0}
                    color="purple"
                    size="sm"
                    aria-label={`${l.label || `库 #${l.library_id}`} 已看 ${l.watched}`}
                  />
                </div>
              ))}
            </Stack>
            ...
            <Stack gap="xs">
              {stats.by_format.map((f) => (
                <div key={f.format || 'unknown'}>
                  ...
                  <Progress
                    value={formatMax > 0 ? (f.watched / formatMax) * 100 : 0}
                    color="purple"
                    size="sm"
                    aria-label={`${f.format || '未知格式'} 已看 ${f.watched}`}
                  />
                </div>
              ))}
            </Stack>
              ...
              <Card key={m.id} withBorder padding="xs" radius="md">
                <Group justify="space-between" wrap="nowrap">
                  <Group gap="sm" wrap="nowrap" style={{ minWidth: 0 }}>
                    <Text fw={700} c="dimmed" w={24} ta="center">
                      #{i + 1}
                    </Text>
                    <Link to={`/play/${m.id}`} style={{ textDecoration: 'none', minWidth: 0 }}>
                      <Group gap="sm" wrap="nowrap" style={{ minWidth: 0 }}>
                        ...
                        <Text size="sm" truncate title={mediaDisplayName(m)}>
                          {mediaDisplayName(m)}
                        </Text>
                      </Group>
                    </Link>
                  </Group>
                  <Badge color="purple" variant="light" style={{ flexShrink: 0 }}>
                    {m.view_count ?? 0} 次
                  </Badge>
                </Group>
                <Progress
                  value={topMax > 0 ? ((m.view_count ?? 0) / topMax) * 100 : 0}
                  color="purple"
                  size="xs"
                  mt={6}
                  aria-label={`${mediaDisplayName(m)} 观看 ${m.view_count ?? 0} 次`}
                />
              </Card>
            ))}
          </Stack>
...
function MediaTab({
  ...
  return (
    <Stack gap="lg">
      <Text size="sm" c="dimmed">
        媒体库整体规模与随时间的增长：媒体总量、构成、累计容量与时长。
      </Text>
            ...
            <Stack gap="xs">
              ...
              <Group gap="lg">
                <Group gap={6}>
                  ...
                  <Text size="sm">{summary.video_count.toLocaleString()}</Text>
                </Group>
                <Group gap={6}>
                  <Badge color="grape" variant="filled" size="sm">
                    图片
                  </Badge>
                  <Text size="sm">{summary.image_count.toLocaleString()}</Text>
                </Group>
              </Group>
            </Stack>
          )}
            ...
            <Stack gap="xs">
              {byLibrary.map((l) => (
                <div key={l.library_id}>
                  ...
                  <Progress
                    value={libMax > 0 ? (l.media_count / libMax) * 100 : 0}
                    color="purple"
                    size="sm"
                    aria-label={`${l.label || `库 #${l.library_id}`} 媒体 ${l.media_count} 个`}
                  />
                </div>
              ))}
            </Stack>
