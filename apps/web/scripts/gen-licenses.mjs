...
//
// 用法：node apps/web/scripts/gen-licenses.mjs（或 npm --prefix apps/web run gen:licenses）
...
// 仓库根目录（apps/web/scripts 的上三级）
const repoRoot = join(scriptDir, '..', '..', '..');
const webDir = join(scriptDir, '..');
const outputPath = join(webDir, 'src', 'data', 'licenses.json');
...
function buildFrontendLicenses() {
    ...
    const selfPkg = JSON.parse(readFileSync(join(webDir, 'package.json'), 'utf8'));
    const selfKey = `${selfPkg.name}@${selfPkg.version}`;
    const packages = new Promise((resolve, reject) => {
        licenseChecker.init(
            { start: webDir, production: true, excludePackages: selfKey },
            (err, pkgs) => {
                if (err) reject(err);
                else resolve(pkgs);
            },
    ...
    return packages.then((pkgs) =>
        Object.entries(pkgs)
            .map(([key, info]) => {
                ...
                const license = Array.isArray(licenseField)
                    ? licenseField.join(', ')
                    : licenseField || '';
                const text =
                    info.licenseFile && existsSync(info.licenseFile)
...
function guessLicenseName(text) {
    ...
    if (
        head.includes('bsd 3-clause') ||
        (head.includes('redistribution') && head.includes('neither the name'))
    )
        return 'BSD-3-Clause';
    ...
    return deps
        .map(({ name, version }) => {
            ...
            return {
                ...
                version,
                license: '',
                url: `https://pkg.go.dev/${name}@${version}?tab=licenses`,
            };
        })
        .sort((a, b) => a.name.localeCompare(b.name));
}
...
async function main() {
    ...
    console.log(
        `  后端依赖：${backend.length} 项（含全文 ${backendWithText} 项，余下给 pkg.go.dev 链接）`,
    );
}
