...
describe("wiki app", () => {
  ...
  it("支持搜索、分组、场景切换与状态预览", () => {
    ...
    expect(
      container.querySelector('[data-testid="wiki-preview-hls-preview-card"]')
        ?.textContent,
    ).toContain("HLS");
    ...
    expect(
      container.querySelector('[data-testid="wiki-preview-task-status"]')
        ?.textContent,
    ).toContain("转码任务");

    const scenario = requireSelect(
      container,
    ...
    expect(
      container.querySelector('[data-testid="wiki-selected-scenario"]')
        ?.textContent,
    ).toContain("转码失败");
    ...
    expect(
      container.querySelector('[data-testid="wiki-active-state"]')?.textContent,
    ).toContain("error");
    expect(
      container.querySelector('[data-testid="wiki-snippet-task-status"]')
        ?.textContent,
    ).toContain("@jianvideo/ui");
