...
import {
  createSnippet,
  uiPreviewCatalog as sharedUiPreviewCatalog,
  type UiPreviewDescriptor,
} from "@jianvideo/ui";

export type WikiGroupId =
  "basic" | "media" | "task" | "space" | "pixi" | "theme";
...
export function filterWikiCatalog(
  filter: WikiCatalogFilter,
): readonly UiPreviewDescriptor[] {
...
export function getWikiPixiMetricSummary(): string {
  ...
  const memoryMb = Math.round(
    estimateTextureMemoryBytes(144, 160, 90) / 1024 / 1024,
  );
