...
func NewHealthService(db *gorm.DB, checks mediaHealthChecks) *HealthService {
	return &HealthService{repo: newGormHealthRepository(db), checks: checks, states: make(map[string]healthSpaceState)}
}
...
func (s *HealthService) ListIssuesInSpace(spaceID string) ([]models.MediaHealthIssue, error) {
	return s.repo.ListIssues(spaceID)
}
...
func (s *HealthService) runScanInSpace(spaceID string) error {
	...
	media, err := s.repo.ListActiveMedia(spaceID)
	if err != nil {
	...
	if err := s.repo.ReplaceIssues(spaceID, found); err != nil {
		s.setError(spaceID, err)
		return err
	}
