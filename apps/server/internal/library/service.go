...
type Service struct {
	...
	pathRepo                     libraryPathRepository
	...
	mediaTypeRuleRepo            mediaTypeRuleRepository
...
func NewService(db *gorm.DB) *Service {
	return &Service{
		db: db, mediaRepo: newGormMediaRepository(db), pathRepo: newGormLibraryPathRepository(db),
		spaceRepo: newGormSpaceRepository(db), metadataRepo: newGormMetadataRepository(db),
		chapterRepo: newGormChapterRepository(db), bookmarkRepo: newGormBookmarkRepository(db),
		tagRepo: newGormTagRepository(db), albumRepo: newGormAlbumRepository(db),
		coverRepo: newGormCoverRepository(db), viewRepo: newGormViewRepository(db),
		watchRepo: newGormWatchStateRepository(db), inferenceRepo: newGormInferenceRepository(db),
		mediaTypeRuleRepo: newGormMediaTypeRuleRepository(db),
...
func (s *Service) CreateLibraryPathWithKindInSpace(spaceID, path, dirType, label, libraryKind string) (*models.LibraryPath, error) {
	...
	if err := s.pathRepo.RunInTx(func(tx *gorm.DB) error {
		if err := s.pathRepo.CreateTx(tx, lp); err != nil {
			return err
		}
...
func (s *Service) ListLibraryPathViewsInSpace(spaceID string) ([]PathView, error) {
	...
	countByLibrary, err := s.mediaRepo.CountActiveMediaByLibrary(spaceID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetLibraryPathByIDInSpace(spaceID string, id int64) (*models.LibraryPath, error) {
	return s.pathRepo.GetByID(spaceID, id)
}
...
func (s *Service) UpdateLibraryPathWithKindInSpace(spaceID string, id int64, label *string, enabled *bool, libraryKind *string) (*models.LibraryPath, error) {
	...
	if err := s.pathRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.pathRepo.GetByIDTx(tx, spaceID, id)
		if err != nil {
			return err
		}
		...
		if err := s.pathRepo.UpdateTx(tx, spaceID, id, updates); err != nil {
			return err
		}
		got, err := s.pathRepo.GetByIDTx(tx, spaceID, id)
		if err != nil {
			return err
		}
		after = *got
...
func (s *Service) spaceIDForLibrary(libraryID int64) (string, error) {
	if !s.pathRepo.HasTable() {
		return models.DefaultSpaceID, nil
	}
	spaceID, err := s.pathRepo.GetSpaceIDByLibraryID(libraryID)
	if err != nil {
...
func (s *Service) deleteLibraryPathInSpaceOnce(spaceID string, id int64) error {
	return s.pathRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.pathRepo.GetByIDTx(tx, spaceID, id)
		if err != nil {
			return err
		}
		if err := s.mediaRepo.DeleteHardByLibraryIDTx(tx, spaceID, id); err != nil {
			return err
		}
		if err := s.pathRepo.DeleteExtensionsByLibraryIDTx(tx, id); err != nil {
			return err
		}
		rows, err := s.pathRepo.DeleteTx(tx, spaceID, id)
		if err != nil {
			return err
		}
		if rows == 0 {
...
func (s *Service) CreateMediaFileInSpace(spaceID string, libraryID int64, filePath string, fileSize int64) (*models.MediaFile, error) {
	...
	if err := s.mediaRepo.Create(mf); err != nil {
		return nil, err
	}
...
func (s *Service) ListThumbnailCandidates(spaceID string, afterID int64, limit int) ([]models.MediaFile, error) {
	...
	return s.mediaRepo.ListThumbnailCandidates(spaceID, afterID, limit)
}
...
func (s *Service) DeleteMediaFileInSpace(spaceID string, id int64) error {
	...
	return s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.mediaRepo.GetActiveForSoftDeleteTx(tx, spaceID, id)
		if err != nil {
			return fmt.Errorf("媒体文件不存在")
		}
		if err := s.mediaRepo.SoftDeleteTx(tx, spaceID, id, now); err != nil {
			return err
		}
		return s.recordAuditTx(tx, audit.EventInput{
			Scope:        audit.ScopeSpace,
...
func (s *Service) BatchDeleteMediaFilesInSpace(spaceID string, ids []int64) (int64, error) {
	...
	err := s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.mediaRepo.ListActiveByIDsTx(tx, spaceID, ids)
		if err != nil {
			return err
		}
		if len(before) == 0 {
		...
		rows, err := s.mediaRepo.SoftDeleteByIDsTx(tx, spaceID, validIDs, now)
		if err != nil {
			return err
		}
		affected = rows
...
func (s *Service) BatchReassignLibraryInSpace(spaceID string, ids []int64, targetLibraryID int64) (BatchReassignLibraryResult, error) {
	...
	err := s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		...
		rows, err := s.mediaRepo.ReassignLibraryByIDsTx(tx, spaceID, validIDs, targetLibraryID)
		if err != nil {
			return err
		}
		out.Moved = rows
...
func (s *Service) GetMediaFilesByIDsInSpace(spaceID string, ids []int64) ([]models.MediaFile, error) {
	return s.mediaRepo.ListByIDsActive(spaceID, ids)
}
...
func (s *Service) ListDeletedMediaFilesInSpace(spaceID string) ([]models.MediaFile, error) {
	return s.mediaRepo.ListDeleted(spaceID)
}
...
func (s *Service) RestoreMediaFileInSpace(spaceID string, id int64) error {
	spaceID = normalizeSpaceID(spaceID)
	return s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.mediaRepo.GetDeletedForRestoreTx(tx, spaceID, id)
		if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		rows, err := s.mediaRepo.ClearDeletedAtTx(tx, spaceID, id, before.DeletedAt)
		if err != nil {
			return err
		}
		if rows != 1 {
...
func (s *Service) RenameMediaFileInSpace(spaceID string, id int64, newName string) (*models.MediaFile, error) {
	...
	if err := s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		if _, err := s.mediaRepo.UpdateFieldsTx(tx, spaceID, id, updates); err != nil {
			return err
		}
		return s.recordAuditTx(tx, audit.EventInput{
			Scope:        audit.ScopeSpace,
...
func (s *Service) WritebackMediaMetadataInSpace(spaceID string, id int64) (*models.MediaFile, error) {
	...
	if err := s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		...
		if _, err := s.mediaRepo.UpdateFieldsTx(tx, spaceID, id, metadataWritebackUpdates(&after)); err != nil {
			return err
		}
...
func (s *Service) UpdateDisplayNameInSpace(spaceID string, id int64, displayName string) (*models.MediaFile, error) {
	...
	rows, err := s.mediaRepo.UpdateField(spaceID, id, "display_name", displayName)
	if err != nil {
		return nil, err
	}
	if rows == 0 {
		// 区分「媒体不存在」与「值未变化」：再查一次确认存在性
		count, err := s.mediaRepo.CountByID(spaceID, id)
		if err != nil {
			return nil, err
		}
		if count == 0 {
...
func (s *Service) UpdateMediaNotesInSpace(spaceID string, id int64, notes string) (*models.MediaFile, error) {
	...
	rows, err := s.mediaRepo.UpdateField(spaceID, id, "notes", notes)
	if err != nil {
		return nil, err
	}
	if rows == 0 {
...
func (s *Service) GetMediaFileByPathInSpace(spaceID, filePath string) (*models.MediaFile, error) {
	return s.mediaRepo.GetMediaFileByPath(spaceID, filePath)
}
...
func (s *Service) GetMediaFileByLibraryAndPathInSpace(spaceID string, libraryID int64, filePath string) (*models.MediaFile, error) {
	return s.mediaRepo.GetMediaFileByLibraryAndPath(spaceID, libraryID, filePath)
}
...
func (s *Service) getMediaFileAnyStateByLibraryAndPathInSpace(spaceID string, libraryID int64, filePath string) (*models.MediaFile, error) {
	return s.mediaRepo.GetByLibraryAndPathIncludingDeleted(spaceID, libraryID, filePath)
}
...
func (s *Service) DeleteMediaFileByPath(filePath string) error {
	filePath = filepath.ToSlash(filePath)
	return s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		return s.mediaRepo.DeleteHardByPathTx(tx, filePath)
	})
}
...
func (s *Service) DeleteMediaFileByLibraryAndPath(libraryID int64, filePath string) error {
	filePath = filepath.ToSlash(filePath)
	return s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		return s.mediaRepo.DeleteHardByLibraryAndPathTx(tx, libraryID, filePath)
	})
}
...
func (s *Service) MarkMediaMissingByLibraryAndPath(spaceID string, libraryID int64, filePath string) error {
	...
	err := s.mediaRepo.RunInTx(func(tx *gorm.DB) error {
		before, err := s.mediaRepo.GetActiveByLibraryAndPathTx(tx, spaceID, libraryID, filePath)
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return nil
			}
		...
		if _, err := s.mediaRepo.UpdateFieldsTx(tx, spaceID, before.ID, map[string]any{
			"file_state": models.MediaFileStateMissing,
		}); err != nil {
			return err
		}
		if err := s.recordAuditTx(tx, audit.EventInput{
...
func (s *Service) applyRenameChange(change ScanChange) (int, error) {
	if change.OldPath != "" {
		mf, err := s.mediaRepo.GetMediaFileByLibraryAndPathAnyState(change.SpaceID, change.LibraryID, change.OldPath)
		if err == nil {
...
func (s *Service) updateExistingMediaFromPath(mf *models.MediaFile, newPath string, change ScanChange) error {
	...
	if err := s.mediaRepo.UpdateFieldsByID(mf.ID, updates); err != nil {
		return err
	}
...
func (s *Service) ScanContextForLibraryInSpace(spaceID string, libraryID int64) (ScanContext, error) {
	...
	rawKind, err := s.pathRepo.GetLibraryKind(scanCtx.SpaceID, libraryID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return scanCtx, nil
	}
	if err != nil {
	...
	kind, err := normalizeLibraryKind(rawKind)
	if err != nil {
		return ScanContext{}, err
	}
...
func (s *Service) reconcileMissingInSpace(spaceID string, libraryID int64) error {
	...
	missingCount := 0
	for {
		rows, err := s.mediaRepo.ListActiveForReconcile(spaceID, libraryID, lastID, reconcileBatchSize)
		if err != nil {
			return err
		}
		if len(rows) == 0 {
...
func (s *Service) indexMediaFiles(scanCtx ScanContext, paths []string) (int, error) {
	...
	// 批量查询已有记录，避免 N+1 查询
	existingFiles, err := s.mediaRepo.ListByLibraryAndPaths(scanCtx.SpaceID, scanCtx.LibraryID, normalizedPaths)
	if err != nil {
		return 0, err
	}
...
func (s *Service) indexSMBMediaFiles(spaceID string, libraryID int64, paths []string, smbFS *smb.FS) (int, error) {
	existingFiles, err := s.mediaRepo.ListByLibraryAndPaths(spaceID, libraryID, paths)
	if err != nil {
		return 0, err
	}
...
func (s *Service) AddMediaExtensionInSpace(spaceID string, libraryID int64, extension, mediaType string) error {
	...
	return s.pathRepo.FirstOrCreateExtension(&item)
}
...
func (s *Service) DeleteMediaExtensionInSpace(spaceID string, libraryID int64, extension string) error {
	...
	rows, err := s.pathRepo.DeleteExtension(libraryID, ext)
	if err != nil {
		return err
	}
	if rows == 0 {
...
func (s *Service) listCustomMediaExtensions(libraryID int64) ([]models.MediaExtension, error) {
	return s.pathRepo.ListCustomExtensions(libraryID)
}
