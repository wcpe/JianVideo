...
// SummaryFormatRow 按 library+format 分组的聚合行（FR2-070 续18 / FR-117 规则口径）。
...
func (s *Service) fillSummaryLibraryCount(spaceID string, summary *Summary) error {
	count, err := s.pathRepo.CountEnabledBySpace(spaceID)
	if err != nil {
		return err
	}
...
func (s *Service) fillRuleBasedSummary(spaceID string, summary *Summary) error {
	rows, err := s.mediaRepo.ListSummaryFormatRows(spaceID)
	if err != nil {
		return err
	}
...
func (r *gormMediaRepository) ListSummaryFormatRows(spaceID string) ([]SummaryFormatRow, error) {
